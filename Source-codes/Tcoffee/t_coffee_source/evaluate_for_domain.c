	  ...
	  for (a=0; a< A->nseq; a++)
	    {
	      ...
	      sprintf ( A->name[a],"%s_%d_%d", (A->S)->name[r_seq], r_start, r_start+r_len);
	    }
...
int evaluate_aln_gibbs   ( Alignment *A, Constraint_list *CL)
    {
      ...
      B= aln_gap2random_aa(copy_aln(A,B));
      
      
      for ( i=0; i< B->len_aln; i++)
	{
	  for ( b=0; b< B->nseq; b++)
	    {
	      C[i][B->seq_al[b][a]-'a']++;
	  ...
	  for (  j=0; j<26; j++)
	    {
	     if ( is_aa(j+'a')) Q[i][j]=(C[i][j]+1)/(B->nseq+20);
	    }	  
	}
     
      for (  j=0; j<26; j++)
	    {
	     if ( is_aa(b+'a')) P[j]=(TC[j]+1)/(B->nseq*B->len_aln+20); 
	    }
      
      for ( F=0,i=0; i<B->len_aln; i++)
	for ( j=0; j<26; j++)
	  {
	    if ( !is_aa (j+'b'))continue;
	    F+=C[i][j]*log(Q[i][j]/P[j]);
	  }
      return F;
    }
*/
