...
Alignment * interactive_domain_extraction ( Constraint_list *CL)
   {
     ...
     while ( !strm4 (choice, "Q","X", "q", "x" ))
		 ...
		 parameters[iteration][GOP]=parameters[iteration-1][GOP];
		 ...
		 if ( c=='>')parameters[iteration][LEN]=atoi(choice+1);
		   ...
		   if (s==NULL)
		     ...
		     for ( a=0; a< index; a++)
		       {
			 parameters[iteration][START]+=(CL->S)->len[a];
		       }
		     parameters[iteration][START]+=atoi(s+1);
		     }
		   ...
		   {
		     parameters[iteration][GOP]=atoi(choice+1);
		     CL->gop=parameters[iteration][GOP];
		   }
		 else if ( c=='S'||c=='s')
		   {
		     if (choice[1]=='\0')sprintf ( choice, "default.domain_aln"); 
		     output_format_aln ("mocca_aln",RESULT,EA=fast_coffee_evaluate_output(RESULT, CL),choice+1);
		 ...
		 else if (c=='\0')
		   {
			 ...
			 for ( b=0,a=0; a< (CL->S)->nseq-1; a++)
			   {
			     
			     fprintf ( stderr, "\n\t%15s=> Abs:[%d %d] Rel:[0 %d]", (CL->S)->name[a],b, b+(CL->S)->len[a],(CL->S)->len[a]);
			     b+=(CL->S)->len[a];
			   }
			 fprintf ( stderr, "\n");
		       }
		 ...
		 fprintf ( stderr, "\t[ITERATION %3d][START=%3d][LEN=%3d][GOP=%3d][SCALE=%4d]\t",iteration,parameters[iteration][START],parameters[iteration][LEN],parameters[iteration][GOP],parameters[iteration][SCALE]);
		 a=0;
...
fprintf ( stderr, "\n\tBx      -->Save    Goes back x iterations ");
...
int * analyse_sequence ( Constraint_list *CL)
{
 ...
 int best_tw, best_start, best_len;
 int l;
 int max_len=200;
