...
/* read the seqno_th. sequence from a PILEUP multiple alignment file */
{
	...
	int i,j,k;
	unsigned char c;
	if ( !line)line=calloc ( (MAXLINE+1), sizeof (char));
...
int readseqs(char *saga_file,char ***SAGA_SEQ, char*** SAGA_NAMES, int ***SAGA_LEN) /*first_seq is the #no. of the first seq. to read */
{
	...
	if ( !line) line=calloc   ( (FILENAMELEN+1), sizeof (char));
	if ( !seq1) seq1=calloc   ( (SEQ_MAX_LEN+2), sizeof (char));
	if ( !sname1)sname1=calloc ( (MAXNAMES+1), sizeof (char));
	if ( !title)title=calloc  ( (MAXTITLES+1), sizeof (char));
