...
static Nlm_FloatHi BLOSUM62_JOINT_PROBS[Alphsize][Alphsize] = {
...
record_joint_probs(char *matrixName,
                   NRitems * NRrecord)
{
    Nlm_FloatHi sum;            /*sum of frequency ratios, should be 1 */
...
void
assign_frequencies(Int4 length,
                   Nlm_FloatHi * probArray,
                   Nlm_FloatHi * one_seq_freq,
                   Nlm_FloatHi * background_probs,
                   Nlm_FloatHi * input_probs,
                   Int4 pseudocounts)
{
    Int4 i;                     /* loop index */
    Nlm_FloatHi weight;         /* weight assigned to pseudocounts */
    Nlm_FloatHi sum;            /* sum of initial probabilities */
...
initializeNRprobabilities(NRitems * NRrecord,
                          char *matrixName)
{
    Nlm_FloatHi re_o_implicit = 0.0;    /* implicit relative entropy
                                           of starting matrix */
    Int4 i, j;                          /* loop indices */
...
RE_interface(char *matrixName,
             ...
             Int4 pseudocounts,
             Nlm_FloatHi specifiedRE,
             NRitems * NRrecord)
{

    Int4 i;                     /*loop indices */
    Nlm_FloatHi re_o_newcontext = 0.0;  /*relative entropy implied by
                                          input single sequence
                                          probabilities */
    static Int4 total_iterations = 0;   /* total iterations among all
                                           calls to
                                           compute_new_score_matrix */
    Int4 new_iterations = 0;    /*number of iterations in the most
                                   recent call to
                                   compute_new_score_matrix */
    static Int4 max_iterations = 0; /*maximum number of iterations
                                      observed in a call to
                                      compute_new_score_matrix */

    Nlm_FloatHi lambdaComputed = 1.0;   /*lambda computed by compute_lambda */
...
void disp_RE_data(NRitems * NRrecord)
{
    int j;
     ...
     /*next three variables are arguments to pass to
       compute_new_score_matrix */
    Nlm_FloatHi H_01, H_0B, H_1B;       /*relative entropies */
    Nlm_FloatHi ratio, corr_factor;     /*ratio and correlation of
                                          relative entropies */
   /* RE between first seq and BLOSUM 62 background */
    H_0B = Get_RE(NRrecord->first_seq_freq, NRrecord->first_standard_freq);
    H_1B = Get_RE(NRrecord->second_seq_freq, NRrecord->second_standard_freq);
    /* RE between first seq and BLOSUM 62 background */
    H_01 = Get_RE(NRrecord->second_seq_freq, NRrecord->first_seq_freq);
