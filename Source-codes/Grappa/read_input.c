...
void read_data (FILE *input, struct genome_struct **genome_list,
  ...
  while (a != EOF) {
	...
	if (readingName) {
	  if (buf[0] != '>') {
	    ...
	    fprintf(stderr,"Expecting format: \">Name\" \n");
	    exit(-1);
	  }
	  if (INVDIST) readingName = 0;
	  NumGenomes++;
	}
	else {
	  i = 0;
	  ...
	  if (NumGenes < 0)
	    ...
	    if (NumGenes != gnum) {
	      fflush(outfile);
	      fclose(outfile);
	      fprintf(stderr,"ERROR: read_data() inconsistent num of genes\n");
  ...
  *genome_list =
    (struct genome_struct *)malloc(NumGenomes*sizeof(struct genome_struct));
  ...
  while (a != EOF) {
	...
	if (readingName) {    
	  ...
	  if (strlen(buf+1) > MAX_STR_LEN)
	    fprintf(stderr,"ERROR: Buffer overflow in genome name\n");
	  strcpy((*genome_list)[genome_num].gnamePtr, buf+1);
	  if (INVDIST) {
	  	  fscanf(input, "%s", buf);
	          strcpy((*genome_list)[genome_num].parent, buf);
          }
	  ...
	  if ((*genome_list)[genome_num].genes == NULL)
	    fprintf(stderr,"ERROR: cannot allocate genes\n");
	  ...
	  while (i < strlen(buf)) {
	    ...
	    while ((buf[i] != ' ') && (buf[i] != '\0'))
	      genebuf[g++] = buf[i++];
	    genebuf[g] = '\0';
	    (*genome_list)[genome_num].genes[gnum] = atoi(genebuf);
	    gnum++;
	    g=0;
	    i++;
	  }
...
#endif
/*      if (NumGenomes != 2) {
	fprintf(stderr, "WARNING: NUM_GENOMES == %3d ",NumGenomes);
	fprintf(stderr, "  Only the first two genomes will be used\n\n");
      }*/
      score = 0;
      for (j = 0; j < NumGenomes; j++) {
          if (strcmp((*genome_list)[j].gnamePtr, "Root") == 0) continue;
          for (i = 0; i < NumGenomes; i++) {
	      if (strcmp((*genome_list)[j].parent, 
	                 (*genome_list)[i].gnamePtr) == 0) break;
	  }
	  if (CIRCULAR) {
             score += invdist_circular_nomem(&(*genome_list)[j], 
                                             &(*genome_list)[i], NumGenes);
	  }
	  else {
             score += invdist_noncircular_nomem(&(*genome_list)[j], 
	                                       &(*genome_list)[i],
					      0, NumGenes);
	  }
      }
  ...
  *orig_num_genes = NumGenes;
  ...
  if (NumGenomes >= 3)
    condense_genes(*genome_list, NumGenomes, &NumGenes, CIRCULAR,
		   condense_succ+NumGenes, condense_decode+NumGenes);
