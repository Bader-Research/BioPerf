...
void printedgescores(struct tNode *tree) {
  if (tree == NULL) return; 
  if (tree->lChild) {
...
int iterate_over_tree(int COND,struct tNode *tree,int num_genes,int num_genomes,
		      int tspsolver,int thresh,int score,
		      ...
		      int distmethod,distmem_t *distmem, int correction)
{
  ...
  struct tNode *condnode1=NULL,*condnode2=NULL,*condnode3=NULL,*condmedian=NULL;
  int IMPROVED = TRUE;
  int ncount;
  int relabel = 0;
  ...
  while (IMPROVED){
    ...
    dfs_label_tree(COND,tree,tree,&best_so_far,&IMPROVED,
		   num_genes,num_genomes,weights,&relabel,tspsolver,thresh,
		   adj_list,adj_pool,neighbors,stack,incycle,outcycle,degree,
		   otherEnd,edges,tpool,labels,
		   pred1+num_genes,pred2+num_genes,picked+num_genes,decode,
...
{ 
  ...
  if (!tree_node->lChild->leaf) {
    dfs_label_tree(COND,root,tree_node->lChild,best_so_far,IMPROVED,
		   num_genes,num_genomes,weights,relabel,tspsolver,thresh,
		   adj_list,adj_pool,neighbors,stack,incycle,outcycle,degree,
		   otherEnd,edges,tpool,labels,pred,succ,code,decode,
		   condmedian,condnode1,condnode2,condnode3,
		   genes,CIRCULAR,distmethod,distmem, correction);
  }
  if (tree_node->rChild) {
    if (!tree_node->rChild->leaf) {
      dfs_label_tree(COND,root,tree_node->rChild,best_so_far,IMPROVED,
		     num_genes,num_genomes,weights, relabel,tspsolver,thresh,
		     adj_list,adj_pool,neighbors,stack,incycle,outcycle,degree,
		     otherEnd,edges,tpool,labels,pred,succ,code,decode,
		     condmedian,condnode1,condnode2,condnode3,
  ...
  if (tree_node->need_change > 0 ){ /* internal node needs to be relabeled */
 
    if (COND) { /* condensation */
      ...
      if (num_cond==0)  { /* three identical neighbors... */
	    /* note: only works for circular genome; for linear, value is 1 */
	    for (i=0; i<num_genes; i++) { /* so just copy on in place */
	      tree_node->genome->genes[i] = tree_node->parent->genome->genes[i];
	    }
      ...
      case TSP_BBTSP:
        ...
        convert2_to_tsp(condnode1->genome,condnode2->genome,condnode3->genome,
		        adj_list,adj_pool,num_cond, CIRCULAR);

        score = bbtsp(2*num_cond,condmedian->genome->genes,
		      FALSE, /* cannot use median that does not exist */
                      condnode1->genome->genes,condnode2->genome->genes,
		      condnode3->genome->genes,
		      adj_list,neighbors,stack,outcycle,degree,otherEnd,edges,
		      CIRCULAR);
        break;
#ifdef CONCORDE
      case TSP_CHLINKERN:
        /* creates a weight matrix -- more expensive! */
        convert_to_tsp(condnode1->genome,condnode2->genome,condnode3->genome,
		       num_cond,CIRCULAR,weights);
        score = chlinkern(2*num_cond,weights,condmedian->genome->genes,
		          incycle,outcycle);
        break;
      case TSP_GREEDYLK:
        /* creates a weight matrix -- more expensive! */
        convert_to_tsp(condnode1->genome,condnode2->genome,condnode3->genome,
		       num_cond,CIRCULAR,weights);
        score = greedylk(2*num_cond,weights,condmedian->genome->genes,
		         incycle,outcycle);
        break;
#endif
      case TSP_COALESCED:
        ...
        score = coalestsp(2*num_cond,condmedian->genome->genes,
		      FALSE, /* cannot use median that does not exist */
                      condnode1->genome->genes,condnode2->genome->genes,
		      condnode3->genome->genes,
		      adj_list,neighbors,stack,outcycle,degree,otherEnd,edges,
      ...
      score += (LARGENUM*num_cond);
#ifdef DEBUG
      fprintf(outfile,"for tree node #%3d, new median with score=%5d\n",
	      tree_node->tag,score);
      ...
      switch (distmethod) {
          case DIST_INV:
	        tmp_sc_parent =
	          bp_score(condmedian,condnode1,num_cond,CIRCULAR,distmethod,distmem, correction, num_genes);
	        tmp_sc_lChild =
	          bp_score(condmedian,condnode2,num_cond,CIRCULAR,distmethod,distmem, correction, num_genes);
	        tmp_sc_rChild =
	          bp_score(condmedian,condnode3,num_cond,CIRCULAR,distmethod,distmem, correction, num_genes);
#ifdef DEBUG
	        fprintf(outfile,"new edge scores are %3d, %3d, and %3d\n",
		        tmp_sc_parent,
		        tmp_sc_lChild,
		        tmp_sc_rChild);
	        fprintf(outfile,"ptrs to old edge scores are %p, %p, and %p\n",
          ...
          default:
            ...
            if (delta < 0) { /* get to same state as case DIST_INV */
	          tmp_sc_parent =
      ...
      if (delta < 0) {
	    /* accept the change */
	    if (tree_node->lChild->tag < 0) tree_node->lChild->need_change = 1;
	    if (tree_node->rChild->tag < 0) tree_node->rChild->need_change = 1;
	    if (tree_node->parent->tag < 0) tree_node->parent->need_change = 1;
      ...
      else if (realsolver == INVERSION_MEDIAN) {
/*        decode3(tree_node->genome->genes,condmedian->genome->genes,
	        succ,decode,num_cond);*/
      ...
      for (i = 0; i < num_genes; i++) { /* save old label */
      ...
      case INVERSION_MEDIAN:
          ...
          score = 0;
          if (CIRCULAR) {
              score += invdist_circular(gen[0], tree_node->genome, num_genes, distmem);
              score += invdist_circular(gen[1], tree_node->genome, num_genes, distmem);
              score += invdist_circular(gen[2], tree_node->genome, num_genes, distmem);
          }
          else {
              score += invdist_noncircular(gen[0], tree_node->genome, 0, num_genes, distmem);
              score += invdist_noncircular(gen[1], tree_node->genome, 0, num_genes, distmem);
              score += invdist_noncircular(gen[2], tree_node->genome, 0, num_genes, distmem);
          }
          break;
      case INVERSION_MEDIAN_FAST:
          gen[0] = tree_node->parent->genome;
          ...
          score = 0;
          if (CIRCULAR) {
              albert_inversion_median_circular(gen, num_genes, tree_node->genome->genes);
              score += invdist_circular(gen[0], tree_node->genome, num_genes, distmem);
              score += invdist_circular(gen[1], tree_node->genome, num_genes, distmem);
              score += invdist_circular(gen[2], tree_node->genome, num_genes, distmem);
          }
          else {
              albert_inversion_median_noncircular(gen, num_genes, tree_node->genome->genes);
              score += invdist_noncircular(gen[0], tree_node->genome, 0, num_genes, distmem);
              score += invdist_noncircular(gen[1], tree_node->genome, 0, num_genes, distmem);
              score += invdist_noncircular(gen[2], tree_node->genome, 0, num_genes, distmem);
          }
      ...
      case TSP_BBTSP:
        ...
        score = bbtsp(2*num_genes,tree_node->genome->genes,use_median,
      ...
      case TSP_CHLINKERN:
        ...
        score = chlinkern(2*num_genes,weights,tree_node->genome->genes,
  		          incycle,outcycle);
        break;
      case TSP_GREEDYLK:
        /* creates a weight matrix -- more expensive! */
        convert_to_tsp(tree_node->parent->genome,
  		       ...
  		       tree_node->rChild->genome,
  		       num_genes,CIRCULAR,weights);
        score = greedylk(2*num_genes,weights,tree_node->genome->genes,
  	                 incycle,outcycle);
        break;
#endif
      case TSP_COALESCED:
...
#ifdef DEBUG
          fprintf(outfile,"in labeltree, about to call coalestsp\n");
          fflush(outfile);
#endif
        score = coalestsp(2*num_genes,tree_node->genome->genes,use_median,
                      tree_node->parent->genome->genes,
  		      tree_node->lChild->genome->genes,
  		      tree_node->rChild->genome->genes,
  		      adj_list,neighbors,stack,outcycle,degree,otherEnd,edges,
...
#ifdef DEBUG
      fprintf(outfile,"in labeltree, done with TSP, returned score is %3d\n",score);
      fflush(outfile);
#endif
      tree_node->need_change = 0;
      ...
      case DIST_INV:
	    ...
	    tmp_sc_parent =
	      bp_score(tree_node,tree_node->parent,num_genes,CIRCULAR,
		       distmethod,distmem, correction, num_genes);
	    tmp_sc_lChild =
	      bp_score(tree_node,tree_node->lChild,num_genes,CIRCULAR,
		       distmethod,distmem, correction, num_genes);
	    tmp_sc_rChild =
	      bp_score(tree_node,tree_node->rChild,num_genes,CIRCULAR,
		       distmethod,distmem, correction, num_genes);
    #ifdef DEBUG
	    fprintf(outfile,"new edge scores are %3d, %3d, and %3d\n",
		    tmp_sc_parent,
    ...
    #ifdef DEBUG
            fprintf(outfile,"in labeltree, new (inv) edge scores are %3d, %3d, and %3d, delta is %3d\n",tmp_sc_parent,tmp_sc_lChild,tmp_sc_rChild,delta);
          fflush(outfile);
    #endif
        break;
      case DIST_BP:
  ...
  if (treeNode->lChild) {
    *(treeNode->lChild->sc_parent) = *(treeNode->sc_lChild) =
    ...
    score += *(treeNode->sc_lChild) +
      score_tree(treeNode->lChild,num_genes,CIRCULAR,distmethod,distmem, correction);
  }
  if (treeNode->rChild) {
    *(treeNode->rChild->sc_parent) = *(treeNode->sc_rChild) =
    ...
    score += *(treeNode->sc_rChild) +
      score_tree(treeNode->rChild,num_genes,CIRCULAR,distmethod,distmem, correction);
  }
  return score;
}

int bp_score(struct tNode *node1, struct tNode *node2,int num_genes,
	     int CIRCULAR, int distmethod, distmem_t *distmem, int correction, int oldngenes)
{
  int dist;
  ...
  case DIST_INV:
      if (correction) {
        if (CIRCULAR)
          dist = ede(invdist_circular(node1->genome,node2->genome,num_genes,
			          distmem), oldngenes);
        else
          dist = ede(invdist_noncircular(node1->genome,node2->genome, 0,
				     num_genes, distmem), oldngenes);
      }
      else {
        if (CIRCULAR)
          dist = invdist_circular(node1->genome,node2->genome,num_genes,
			          distmem);
        else
          dist = invdist_noncircular(node1->genome,node2->genome, 0,
				     num_genes, distmem);
      }
    break;
  default:
...
/* Use genome_list to initialize genome pointers in the leaves of the
   tree.
*/
void add_genomes_to_tree(struct tNode *treeNode, struct genome_struct *labels,
...
void label_node(struct tNode *tree_node,struct tNode *new_node,int num_genes) {
}

void leaf_tree(struct tNode *tree) {
  if (tree == NULL) return;
  if (tree->tag < 0) tree->leaf = FALSE;
...
void init_change_bits(struct tNode *tree) {
  if (tree == NULL) return;
  if (tree->tag < 0) { /* internal node */
    tree->need_change = 1;
  }
  else { /* leaf: labels are permanent */
    tree->need_change = 0;
  }
  ...
  int tmp_sc_parent=0, tmp_sc_lChild=0, tmp_sc_rChild=0;
   ...
   switch (realsolver) {
      case INVERSION_MEDIAN:
          ...
          score = 0;
          if (CIRCULAR) {
              score += invdist_circular(gen[0], cNode->genome, num_genes, distmem);
              score += invdist_circular(gen[1], cNode->genome, num_genes, distmem);
              score += invdist_circular(gen[2], cNode->genome, num_genes, distmem);
          }
          else {
              score += invdist_noncircular(gen[0], cNode->genome, 0, num_genes, distmem);
              score += invdist_noncircular(gen[1], cNode->genome, 0, num_genes, distmem);
              score += invdist_noncircular(gen[2], cNode->genome, 0, num_genes, distmem);
          }
          break;
      case INVERSION_MEDIAN_FAST:
          gen[0] = node[0]->genome;
          ...
          score = 0;
          if (CIRCULAR) {
              albert_inversion_median_circular(gen, num_genes, cNode->genome->genes);
              score += invdist_circular(gen[0], cNode->genome, num_genes, distmem);
              score += invdist_circular(gen[1], cNode->genome, num_genes, distmem);
              score += invdist_circular(gen[2], cNode->genome, num_genes, distmem);
          }
          else {
              albert_inversion_median_noncircular(gen, num_genes, cNode->genome->genes);
              score += invdist_noncircular(gen[0], cNode->genome, 0, num_genes, distmem);
              score += invdist_noncircular(gen[1], cNode->genome, 0, num_genes, distmem);
              score += invdist_noncircular(gen[2], cNode->genome, 0, num_genes, distmem);
          }
      ...
      case TSP_BBTSP:
        /* does not create a weight matrix, but sets weight in the adj lists */
        convert2_to_tsp(node[0]->genome, node[1]->genome, node[2]->genome,
                        adj_list,adj_pool,num_genes,CIRCULAR);
      ...
      case TSP_CHLINKERN:
        /* creates a weight matrix -- more expensive! */
        convert_to_tsp(node[0]->genome, node[1]->genome, node[2]->genome,
                       num_genes,CIRCULAR,weights);
        score = chlinkern(2*num_genes,weights,cNode->genome->genes,
                          incycle,outcycle);
        break;
      case TSP_GREEDYLK:
        /* creates a weight matrix -- more expensive! */
        convert_to_tsp(node[0]->genome, node[1]->genome, node[2]->genome,
                       num_genes,CIRCULAR,weights);
        score = greedylk(2*num_genes,weights,cNode->genome->genes,
                         incycle,outcycle);
        break;
#endif
      case TSP_COALESCED:
        ...
        score = coalestsp(2*num_genes,cNode->genome->genes,use_median,
      ...
      if (delta < 0) {
         /* accept the change */
         if (node[1]->tag < 0) node[1]->need_change = 1;
         if (node[2]->tag < 0) node[2]->need_change = 1;
         if (node[0]->tag < 0) node[0]->need_change = 1;
         /* update the distances along the three edges (both ways) */
