...
int gen_new_tree(int *old_tree,int edge,int *new_tree,int n, 
		 int size_of_tree);
int gen_tree(int n,int exp[2*MAX_GENOMES+1],int dif_lev,bin_env_t *env);
...
int inc_bin_tree(int n,bin_env_t *env,int MYPROC,int *dif_lev);
#endif
...
struct tNode  *testtwolevel(int n, struct tNode *tpool, mpz_t PROCS, bin_env_t *env, int* flag,
                            int ** distmatrix,
                            int best_so_far, int minidist);
#else
/* branch up one level*/
struct tNode  *testonelevel(int n, struct tNode *tpool, int PROCS, bin_env_t *env, int* flag,
                            ...
                            int best_so_far, int minidist);
/* branch up two level*/
struct tNode  *testtwolevel(int n, struct tNode *tpool, int PROCS, bin_env_t *env, int* flag,
                            int ** distmatrix, 
                            int best_so_far, int minidist);
#endif
