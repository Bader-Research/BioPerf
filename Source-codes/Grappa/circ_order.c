...
int compute_co_score(int **distmatrix,struct tNode *treeNode,/*int first,*/
		     int *prev/*,struct genome_struct *genome_list,int num_genes*/)
{
  int id, score = 0;
...
int test_all_co_score_fast(int best_so_far_2, int colb, int **distmatrix,struct tNode *root, 
                      struct tNode *treeNode, int firstg,
                      int *prev)
{
    struct tNode* left;
    struct tNode* right;
    ...
    *prev = root->tag;
    ...
    if (tmpColb > colb) colb = tmpColb;

    /* recursive to left child*/
    if (!treeNode->lChild->leaf) {
		tmpColb = test_all_co_score_fast(best_so_far_2, colb, distmatrix,root, 
                          treeNode->lChild, firstg,
                          prev);
        if (best_so_far_2 < tmpColb) {
            /* switch back */
    ...
    treeNode->lChild = left;
    treeNode->rChild = right;
    if (colb < tmpColb) return tmpColb;
    return colb;
}

int test_all_co_score(int best_so_far_2, int colb, int **distmatrix,struct tNode *root, 
                      struct tNode *treeNode, int firstg,
                      int *prev, int flag)
{
    struct tNode* left;
