/* $Id: med_util.c,v 1.8 2001/12/19 22:41:47 acs Exp $
...
distmem_t* new_distmem(int ngenes) {
  distmem_t* distmem = (distmem_t*)malloc(sizeof(distmem_t));

  /* below snipped from GRAPPA */
  distmem->hammingArr = (int *)malloc((ngenes+1)*2*sizeof(int));
  ...
  distmem->greyEdges = (int *)malloc((2*ngenes+2)*sizeof(int));
  ...
  distmem->oriented = (int *)malloc((2*ngenes+2)*sizeof(int));
  ...
  distmem->labeled = (int *)malloc((2*ngenes+2)*sizeof(int));
