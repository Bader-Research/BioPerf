...
/* this is the helper function define in the main.c */
extern int init_score_tree(int COND,struct tNode *tree,int NUM_GENES,int NUM_GENOMES,
...
int growTree(int COND,struct tNode **tree,int NUM_GENES,int NUM_GENOMES,
             ...
             smalledge_t *smalledges, int ** status, triple_t* triple,
             int inittspsolver, int OPT, int initmethod, bin_env_t *bin_env)
{
    ...
    if (*tree == NULL) {
        ...
        *tree = first(3,tpool,0, bin_env);
        (*tree)->leaf = TRUE;
        score = init_score_tree(COND,*tree,NUM_GENES,3,tspsolver,thresh,
    ...
    for (i = 4; i <= NUM_GENOMES; i++) {
        ...
        score = LARGENUM;
        cutPoint = addNewEdge(COND,(*tree)->lChild, *tree, internal, newNode, &score, NUM_GENES,i,tspsolver,thresh,
            adj_list,adj_pool,neighbors,stack,incycle,
            outcycle,weights,degree,otherEnd,edges,
            tpool,labels,pred1,pred2,picked,decode,
            genes,CIRCULAR, distmethod,distmem, CORRECTION, qpool,
        ...
        score = init_score_tree(COND, *tree, NUM_GENES, i, tspsolver, thresh, adj_list, adj_pool,
            neighbors, stack, incycle, outcycle, weights, degree, otherEnd, edges, 
            tpool, labels, pred1, pred2, picked, decode, genes, CIRCULAR, distmethod,
            distmem, CORRECTION, qpool, edgepool, genome_list, smalledges, status,
            triple, inittspsolver, OPT, initmethod);
...
void copyNode(struct tNode* node1, struct tNode* node2, int num_genes) 
{
    int i;
...
struct tNode* addNewEdge(int COND,struct tNode *tree,struct tNode* root, struct tNode* internal,
                         struct tNode* newNode, int* score,
                         ...
                         int inittspsolver, int OPT, int initmethod)
{
    ...
    root->leaf = TRUE;
    /* score the new tree */
    scores[0] = init_score_tree(COND, root, num_genes, num_genomes, tspsolver, thresh, adj_list, adj_pool,
        neighbors, stack, incycle, outcycle, weights, degree, otherEnd, edges, 
        tpool, labels, pred1, pred2, picked, decode, genes, CIRCULAR, distmethod,
        distmem, CORRECTION, qpool, edgepool, genome_list, smalledges, status,
        triple, inittspsolver, OPT, initmethod);
    ...
    /* recursive to child*/
    root->leaf = TRUE;
    lReturn = addNewEdge(COND,tree->lChild, root, internal, newNode, &scores[1], num_genes,num_genomes,tspsolver,thresh,
        adj_list,adj_pool,neighbors,stack,incycle,
        outcycle,weights,degree,otherEnd,edges,
        tpool,labels,pred1,pred2,picked,decode,
        genes,CIRCULAR, distmethod,distmem, CORRECTION, qpool,
        edgepool, genome_list, smalledges, status, triple,
        inittspsolver, OPT, initmethod);
    rReturn = addNewEdge(COND,tree->rChild, root, internal, newNode, &scores[2], num_genes,num_genomes,tspsolver,thresh,
        adj_list,adj_pool,neighbors,stack,incycle,
        outcycle,weights,degree,otherEnd,edges,
        tpool,labels,pred1,pred2,picked,decode,
        genes,CIRCULAR, distmethod,distmem, CORRECTION, qpool,
    ...
    scores[3] = scores[0];
    if (scores[3] > scores[1]) scores[3] = scores[1];
    if (scores[3] > scores[2]) scores[3] = scores[2];
...
void orderGenome(struct genome_struct* genome_list, struct genome_struct* cpgenome, 
                 int** cpdist, int** distmatrix, int num_genes, int num_genomes,
                 int threshHold, int start)
{
    ...
    for (i = 0; i < num_genomes; i++) {
        ...
        for (j = 0; j < num_genomes; j++) {
            ...
            if (cpdist[i][j] > threshHold) cpdist[i][j] = -1;
        }
    }
...
void eliminateGraph(int** dist, int* order, int num, int start)
{
    int i, j;
    ...
    vertexList = (struct graph_vertex**)malloc(sizeof(struct graph_vertex*)*num);
    for (i = 0; i < num; i++) 
    {
...
struct graph_vertex* createVertex(int num_neighbor, int name) {
    ...
    int i;
    newVertex = (struct graph_vertex*)malloc(sizeof(struct graph_vertex));
    newVertex->neighbors = 
        (struct graph_vertex**)malloc(sizeof(struct graph_vertex*)* (1+num_neighbor));
...
struct vertex_stack* createStack() {
    ...
    newStack = (struct vertex_stack*)malloc(sizeof(struct vertex_stack));
    newStack->head = NULL;
    return newStack;
}
struct graph_vertex* stack_pop(struct vertex_stack* aStack) {
    struct stack_node* tmp;
    struct graph_vertex* reVertex;
    if (aStack == NULL || aStack->head == NULL) return NULL;
    tmp = aStack->head;
...
void stack_push(struct vertex_stack* aStack, struct graph_vertex* aVertex) {
    struct stack_node* tmp1, *tmp2;
    if (aStack == NULL || aVertex == NULL) return;
...
int stack_member(struct vertex_stack* aStack,struct  graph_vertex* v){
    struct stack_node* tmp;
    tmp = aStack->head;
    while(tmp != NULL) {
        if (tmp->theVertex == v) return 1;
        tmp = tmp->next;
    }
    return 0;
}

int stack_empty(struct vertex_stack* aStack) {
    if (aStack == NULL || aStack->head == NULL) return 1;
    else return 0;
}

void stack_del(struct vertex_stack* aStack,struct  graph_vertex* v) {
    struct stack_node* tmp1, *tmp2;
    if (aStack == NULL || aStack->head == NULL) return;
...
int neighbor(struct graph_vertex* v1, struct graph_vertex* v2){
    int i;
    for (i = 0; i < v2->num_neighbors; i++) {
        if (v1 == v2->neighbors[i]) return 1;
    }
    return 0;
}
...
void copyTree(struct tNode* tree, struct genome_struct* labels, int* cpSC_lChild,
    ...
    if (tree->lChild != NULL) copyTree(tree->lChild, labels, cpSC_lChild, cpSC_rChild, cpSC_parent, num_genes);
    if (tree->rChild != NULL) copyTree(tree->rChild, labels, cpSC_lChild, cpSC_rChild, cpSC_parent, num_genes);
    return;
}

/* copy 1->2*/
void copyGenome(struct genome_struct* g1, struct genome_struct* g2, int num_genes) {
    int i;
    for (i = 0; i < num_genes; i++) {
...
void restoreTree(struct tNode* tree, struct genome_struct* labels, int* cpSC_lChild,
                 int* cpSC_rChild, int* cpSC_parent, int num_genes) {
    int i;
    if (tree == NULL) return;
    i = tree->tag;
    ...
    *tree->sc_parent = cpSC_parent[i];
    if (tree->lChild != NULL) restoreTree(tree->lChild, labels, cpSC_lChild, cpSC_rChild, cpSC_parent, num_genes);
    if (tree->rChild != NULL) restoreTree(tree->rChild, labels, cpSC_lChild, cpSC_rChild, cpSC_parent, num_genes);
    return;
}
...
int MST(int num_genomes,int** distmatrix)
{
    ...
    us = (struct union_set*)malloc(sizeof(struct union_set)*num_genomes);
    ...
    for (i = 0; i < num_genomes; i++) {
        for (j = i+1; j < num_genomes; j++) {
            if (distmatrix[i][j] > maxweight) maxweight = distmatrix[i][j];
        }
    }
    maxweight+=1;
    ...
    adj_list =
        (struct adj_struct *)malloc((num_genomes)*sizeof(struct adj_struct));
    ...
    for (i = 0; i < num_genomes; i++) {
        if (i == num_genomes - 1 ) {
            ...
            adj_list[i].next = (struct adj_struct *)malloc(sizeof(struct adj_struct));
            node = adj_list[i].next;
        }
        for (j = i+1; j < num_genomes; j++) {
            node->weight = distmatrix[i][j];
            node->vertex = j;
            if (j == num_genomes -1) node->next = NULL;
            else {
                node->next = (struct adj_struct *)malloc(sizeof(struct adj_struct));
                node = node->next;
            }
        }
    ...
    picked = 0;
    newedge = edges;
    while((picked < num_genomes-1) && (newedge < (edges+numedges))) {
        i = newedge->I;
        j = newedge->J;
        i1 = findUF(i, us);
        j1 = findUF(j, us);
            ...
            while(i < num && intersected[i] > 0) {
                ...
                while(j < num && intersected[j] > 0) {
                    ...
                    { 
                        ...
                        over=d_ab-w; 
                        cost=over; 
                        if(d_ab>width) width=d_ab; 
                        if(cost>min_cost) goto CONTINUE; 
                    } 
                    j++;
                } 
                i++;
