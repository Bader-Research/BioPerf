...
int inc_bin_tree(int n,bin_env_t *bin_env,mpz_t inc,int *dif_lev)
{
  int i;
  bin_env_t *tmp_env;
  mpz_t tmp, tmp2;
...
#else
int inc_bin_tree(int n,bin_env_t *bin_env,int inc,int *dif_lev)
{
  int i;
  bin_env_t *tmp_env;
  int tmp;
...
{   bin_env_t *ptr_env,*bin_env;
  ...
  assert_malloc(bin_env,__FILE__,__LINE__-1);
    ...
    for(i=3;i<=n;i++) {
    ptr_env=bin_env+i-3;
    ptr_env->tree_exp=(int *)malloc((2*i-1)*sizeof(int));
...
struct tNode  *first(int n, struct tNode *tpool, int MYPROC, bin_env_t *env) {
  struct tNode *tree;
...
#ifdef GMP
struct tNode  *next(int n, struct tNode *tpool, mpz_t PROCS, bin_env_t *env)
#else
struct tNode  *next(int n, struct tNode *tpool, int PROCS, bin_env_t *env)
#endif
{
  struct tNode *tree;
  int exp[2*MAX_GENOMES+1];
...
{   
  ...
  while (--argc > 0 && (*++argv)[0] == '-'){
	...
	argc--;
	outfilename = 
	  (char *)malloc(MAX_FILENAME_LEN*sizeof(char));
	strcpy(outfilename, *++argv);
	break;
      case 'n':
	n=atoi(*++argv);
	if (n<2) perror("n must be larger or equal to 2\n");
	argc--;
	break;
      case 'h':
...
int gen_new_tree(int *old_tree, int edge,int *new_tree,
...
int gen_tree(int n,int new_tree[2*MAX_GENOMES+1],
  ...
  if (n==3) {
    /*base case*/
    for(i=0;i<env[n-3].size_of_tree;i++) new_tree[i]=0;
...
struct tNode  *exp_to_tree(int * exp,int n, struct tNode *tpool) {
...
#ifdef DEBUG1
  for(i=0;i<2*n-1;i++) fprintf(outfile,"%d,",exp[i]);
  fprintf(outfile,"\n");
#endif
  ...
  /* no need for malloc: use the next slot in the tpool array */
  child=&tpool[1];
  child->parent=tree; tree->lChild=child;
  child->tag=exp[2];
  ...
  for (i=3 ; i<2*n-1 ; i++) {
    /* no need for malloc: use the next slot in the tpool array */
    child=&tpool[i-1];
    child->parent=ptr;  ptr->leaf=FALSE;
    child->tag=exp[i];
    child->lChild=child->rChild=NULL;
    
    if (lchild==1) ptr->lChild=child;
    else  ptr->rChild=child;
...
int tree_to_exp(struct tNode  *tree,char *str) {
  int len,tag,pos;

  if ((tag=tree->tag)>0) {
    sprintf(str,"%d",tree->tag);
    len=1;   
    while((tag=tag/10)>0) len++;
    return len;
  }
  else {
    *str='(';
...
#ifdef GMP
struct tNode  *testonelevel(int n, struct tNode *tpool, mpz_t PROCS, bin_env_t *env, int* flag,
                            int ** distmatrix, 
                            int best_so_far, int minidist)
#else
struct tNode  *testonelevel(int n, struct tNode *tpool, int PROCS, bin_env_t *env, int* flag,
                            int ** distmatrix, 
                            int best_so_far, int minidist)
#endif
{
...
struct tNode  *testtwolevel(int n, struct tNode *tpool, int NUM_GENOMES, bin_env_t *env, int* flag,
                            int ** distmatrix, 
                            int best_so_far, int minidist)
{
  struct tNode *tree;
  ...
  if (gen_tree_levels(n-1,old_trees, tpool,dif_lev,env, NUM_GENOMES, distmatrix, best_so_far, minidist) == -1)
  {
	  *flag = 1;
	  return NULL;
  }
...
/*generated trees are stored in new_trees in ascending order of tree_id,
  old_trees acts as a buffer*/
int gen_tree_levels(int n,int new_tree[2*MAX_GENOMES+1], struct tNode *tpool, 
	     int dif_lev,bin_env_t *env, int NUM_GENOMES, int ** distmatrix, 
         int best_so_far, int minidist) {
  int old_trees[2*MAX_GENOMES+1];
  int i, prev, colb, firstg;
  int treeid;
  ...
  if (n==3) {
    ...
    gen_tree_levels(n-1,old_trees,tpool,dif_lev,env, NUM_GENOMES,distmatrix, best_so_far, minidist);
	if (n == NUM_GENOMES-1) {
	  tree = exp_to_tree(old_trees,n-1,tpool);
	  tree->leaf = TRUE;
	  prev = tree->tag; 
