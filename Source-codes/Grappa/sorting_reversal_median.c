/* $Id: sorting_reversal_median.c,v 1.2 2001/12/20 03:03:59 acs Exp $
...
void find_reversal_median(struct genome_struct *median, 
  ...
  for (MIN_MED = 0, i = 0; i < NPERMS - 1; i++) {
    for (j = i + 1; j < NPERMS; j++) {
      d[i][j] = d[j][i] = 
	invdist_noncircular(gen[i], gen[j], 0, ngenes, mm->distmem);
      MIN_MED += d[i][j];
    }
  }
  MIN_MED = ceil((MIN_MED)/2.0);
  ...
  for (g[0] = NULL, i = 0; i < NPERMS - 1 && g[0] == NULL; i++) {
    for (j = i + 1; j < NPERMS && g[0] == NULL; j++) {
      ...
      int oth2 = (j - i == 1) ? d[i][(j + 1) % NPERMS] : d[i + 1][j];
  ...
  for (pass = 1; pass <= 2; pass++) {
	...
	/* set distance wrt d[1] according to whether a sorting,
        ...
        if (mm->mark[rev->start][rev->stop] & mm->MASK[SORTING][1]) 
          n->d1 = v->d1 - 1;
        else if (mm->mark[rev->start][rev->stop] & mm->MASK[NEUTRAL][1]) 
          n->d1 = v->d1;
        else 
          n->d1 = v->d1 + 1;
	...
	/* same wrt d[2] */
        if (mm->mark[rev->start][rev->stop] & mm->MASK[SORTING][2]) 
          n->d2 = v->d2 - 1;
        else if (mm->mark[rev->start][rev->stop] & mm->MASK[NEUTRAL][2]) 
          n->d2 = v->d2;
        else
          n->d2 = v->d2 + 1;
	...
	/* calculate best and worst possible scores according to
        ...
        n->best_possible_score = n->distance + ceil((n->d1 + n->d2 + 
                                                     distbetween)/2.0);
	...
	for (j = 0; j < NPERMS; j++) {
      ...
      if (pass == 2)
	while ((rev = pop_queue(&mm->candidates)) != NULL) free(rev);
  ...
  if (medmem == NULL) free_median_memory(mm);
}
...
MedianMemory *new_median_memory(int ngenes, int minm, int maxm) {
  ...
  for (i = 0; i < NREVTYPES; i++) 
    for (j = 0; j < NPERMS; j++) 
      init_list(&mm->revs[i][j], (ngenes + 1) * ngenes, sizeof(Reversal*));
  init_list(&mm->candidates, (ngenes + 1) * ngenes, sizeof(Reversal*));
...
void free_median_memory(MedianMemory *mm) {
  int i, j;
  ps_free(mm->ps);
  vf_free(mm->vf);
  for (i = 0; i < NREVTYPES; i++) 
