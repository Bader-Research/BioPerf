...
Hashtable* new_hashtable(int ngenes, int expected_size, 
  ...
  h->bucketsize = ceil(loading_factor * expected_size / (float)h->nbuckets);

  /* Choose the number of digits to sample when calculating hashkeys */
  h->idxdigits = SAMPLE_PERCENTAGE * h->ngenes;
  if (h->idxdigits < MIN_DIGITS) h->idxdigits = MIN_DIGITS;
  if (h->idxdigits > h->ngenes) h->idxdigits = h->ngenes;
...
#ifdef THREADSAFE
  h->rwlock = (mythread_rwlock_t*)calloc(h->nbuckets, sizeof(mythread_rwlock_t));
  if (h->rwlock == NULL) {
    fprintf(stderr, "Error allocating space for hashtable.\n");
    exit(errno);
  }
...
void ht_insert_key(Hashtable* h, int* perm, int k){
  ...
  if (h->table[k] == NULL) {
    h->table[k] = (int*)calloc(h->sizes[k] * h->ngenes, sizeof(int));
    if (h->table[k] == NULL) {
