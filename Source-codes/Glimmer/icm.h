 ...
 * File:         icm.h 
 * Last Updated: Mon Dec  9 14:02:58 EST 2002
 *
 ...
 ******************************************************************/
void 
Fast_Evaluate (char *orf, int length, int window_length,  tModel * Delta0, 
               tModel * Delta1, tModel *Delta2, double & score)
{
  int start = 0, stop = window_length-1, i;
  
  /* ensure there are no inline stop codons before doing any work */
...
float *  Full_Window_Prob_Dist
    (char * s, tModel * delta)

//  Return a pointer to an array of 4 probabilities (of a, c, g, t, resp)
//  for the last character in the window of length  MODEL_LEN
//  conditioned on the previous characters in the window, using the
//  interpolated context model in  delta .

  {
   int  i, pos, num_node = 0;
      ...
      switch  (tolower (Filter (s [pos])))
        {
         ...
         case  't' :
           ...
           break;
         default:
           fprintf (stderr, "switch error .%c. in Full_Window_Prob_Dist\n",
                    s [pos]);
           fprintf (stderr, "pos = %d  window = \"%-.*s\"\n",
                    pos, MODEL_LEN, s);
           exit (EXIT_FAILURE);
        }
     }
   ...
   return  delta [num_node] . prob;
  }



float *  Partial_Window_Prob_Dist
    (char * s, int n, tModel * delta)

//  Return a pointer to an array of 4 probabilities (of a, c, g, t, resp)
//  for the last character in the window  s [0 .. (n - 1)]
//  conditioned on the previous characters in the window, using the
//  interpolated context model in  delta .

  {
   int  i, offset, pos, num_node;
