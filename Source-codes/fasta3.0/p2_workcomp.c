...
/* $Name: fa34t25d2 $ - $Id: p2_workcomp.c,v 1.32 2005/03/14 20:25:57 wrp Exp $ */
...
extern int calcons(unsigned char *aa0, int n0,
	    ...
	    struct a_struct *aln, struct pstruct pst,
	    char *seqc0, char *seqc1, char *seqca, void *f_str);
...
#endif
#ifdef MPI_SRC
      MPI_Recv(aa0[0],qm_msg.n0+1+SEQ_PAD,MPI_BYTE,hosttid,MSEQTYPE1,MPI_COMM_WORLD,
	       &mpi_status);
#endif
#ifdef DEBUG
      /* must have null's at both ends of sequence */
	  ...
	  if (seqc_flag == 2) {
	    if (seqc_buff_cnt < seqc_buff_len - 256) {
	      lc=calc_code(aa0[itt],qm_msg.n0, seqpt[seqnm].aa1,
			   ...
			   aln_dp,pst,seqc,seqc_buff_len-seqc_buff_cnt-10,
			   f_str[itt]);
...
*/
	...
	nc=calcons(aa0[itt],qm_msg.n0, seqpt[seqnm].aa1, seqpt[seqnm].n1,
		   ares,nres,&lc,&aln,pst,seqc0,seqc1,seqca,f_str[itt]);
	memset(seqc0a,' ',nc);
	seqc0a[nc]='\0';
