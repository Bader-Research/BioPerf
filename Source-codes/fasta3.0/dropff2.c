...
static char *verstr="4.2 March 2004 (ajm/wrp)";

int 
shscore(unsigned char *aa0, const int n0, int **pam2, int nsq);
...
int m0_spam(unsigned char *, unsigned char *, int, struct savestr *,
	    int **, struct f_struct *);
int m1_spam(unsigned char *, int, unsigned char *, int, struct savestr *,
	    int **, int, struct f_struct *);

void 
calc_priors(double *priors, struct pstruct *ppst, struct f_struct *f_str,
	    unsigned char *aa1, int n1, int pseudocts);

double
calc_tatusov ( struct slink *last, struct slink *this,
	       const unsigned char *aa0, const int n0, unsigned char *aa1, int n1,
	       int **pam2, int nsq, struct f_struct *f_str, int pseudocts,
	       int do_opt, int zsflag);

int sconn(struct savestr **v, int nsave, int cgap,
	  struct f_struct *, struct rstruct *, struct pstruct *,
	  unsigned char *aa0, int n0, unsigned char *aa1, int n1,
	  int opt_prob);
...
int sconn_a(unsigned char *, int, int,
	    struct f_struct *, struct a_struct *);
...
#ifdef TFAST  
  n10=aatran(aa1,f_str->aa1x,n1,frame);
  do_fastf (aa0, n0, f_str->aa1x, n10, ppst, f_str, rst, &hoff, 1);
#else	/* FASTA */
  do_fastf(aa0, n0, aa1, n1, ppst, f_str, rst, &hoff, 1);
#endif
...
int m1_spam (unsigned char *aa0, int n0, unsigned char *aa1, int n1,
		...
		int **ares, int *nres, struct a_struct *aln)
{
  ...
  *nres = sconn_a (aa0t,n0, ppst->param_u.fa.cgap, f_str,aln);
  free(aa0t);
...
int sconn_a (unsigned char *aa0, int n0, int cgap, 
	     struct f_struct *f_str, struct a_struct *aln)
{
   int     i, si, cmpp (), n;
   unsigned char *aa0p;
   int sx, dx, doff;
	      ...
	      char *al_str, int al_str_n, struct f_struct *f_str)
{
  ...
  while (i0 < aln->max0 || i1 < aln->max1) {
	...
	op_cnt = 1; p_op = 0; fnum = f_str->aa0ti[i0] + 1;
      }
      op = *rp++;
      lenc++;
