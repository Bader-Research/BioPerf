...
/* $Name: fa34t25d2 $ - $Id: mshowbest.c,v 1.27 2005/03/01 17:30:23 wrp Exp $ */
...
)
{
  ...
  int nshow;
  int quiet;
  int r_cont;	/* indicate that additional results are available */
  int r_cont_min;	/* threshold for additional results */
  int r_margin;
  struct beststr *bbp;
  int n1tot;
  char *bp;
  ...
  int n0, n1;
  struct rstruct rst;
  int *res, nres, sw_score, lc, maxc, nident, nsim, ngap;
...
/*   fprintf (fp, "%3d %s\n", qlib,m_msg->libstr); */
  ...
  if (pst.zsflag >= 0) {
	...
	if (m_msg->show_code == 2) {	fprintf(fp," aln_code"); }
	if (m_msg->markx & MX_HTML && m_msg->show_code!=1) { fprintf(fp," -->");}
      }
      fprintf(fp,"\n");
    }
...
l1:
  ...
  if (m_msg->stages>1 || m_msg->markx & MX_M9SUMM) {
    ...
    if (! m_msg->markx & MX_M9SUMM) {
      ...
      /*       r_cont_min = Es_to_s(bbp->escore, m_msg->n0, n1, m_msg->pstat_void); */
      sw_score=
  ...
  if (m_msg->markx & MX_M9SUMM) {
    if (m_msg->show_code != 1) {
      ...
      if (m_msg->show_code == 2 && seqc_len > 0 && seqc != NULL) {
	fprintf(fp,"\t%s",seqc);
