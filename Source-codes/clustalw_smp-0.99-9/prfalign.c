 ...
 *      * added functions pdiff_reverse() and pdiff_forward()
 *      * changed pdiff() to facilitate threading
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <math.h>
#include "clustalw.h"
#include "pth_measure.h"
#define ENDALN 127

#define THRESHOLD_0 40
#define THRESHOLD_1 70
#define THRESHOLD_2 100
#define THRESHOLD_3 130
#define THRESHOLD_4 160
...
/*
 * The following structure is used to parallellize the first part of pdiff
 */
typedef struct {
    sint A, B, M, N, midi, go1, go2;
    char pad[32];
} pdiff_t;  

/* we need a mutex and a condition for synch/barrier implementation */
pthread_mutex_t pdiff_mtx = PTHREAD_MUTEX_INITIALIZER;
pthread_cond_t pdiff_cond = PTHREAD_COND_INITIALIZER;

pthread_attr_t prf_attr;    /* a thread attribute */
static int threshold;       /* a threshold for pdiff() */
static int pdiff_barrier = 0;

/* 
    sets up the threshold for threading based on the speed of thread
    creation on a given machine
*/
void pdiff_setup_threshold() {
    float thr_create_time;

    thr_create_time = thr_create_usec();
    printf("\n\npthread_create() time (microseconds): %f\n", thr_create_time);
    if ((int)thr_create_time < 100)
        threshold = THRESHOLD_0;
    else if ((int)thr_create_time < 150)
        threshold = THRESHOLD_1;
    else if ((int)thr_create_time < 200)
        threshold = THRESHOLD_2;
    else if ((int)thr_create_time < 250)
...
/* use Myers and Miller to align two sequences */
  ...
  displ = (sint *) ckalloc( (max_aln_length+1) * sizeof (sint) );
    ...
    /* here is where the sum of pairwise alignments happens */
...
void pdiff_reverse(pdiff_t *pd) {
    ...
    tl = 0.0;
    for(j=pd->N-1;j>=0;j--) {
        g = -open_penalty1(pd->A+pd->M,pd->B+j+1);
        tl -= ext_penalty1(pd->A+pd->M,pd->B+j+1);
        RR[j] = g+tl;
        SS[j] = RR[j]-open_penalty2(pd->A+pd->M,pd->B+j);
        gS[j] = open_penalty2(pd->A+pd->M,pd->B+j);
    }

    tl = 0.0;
    for(i=pd->M-1;i>=pd->midi;i--) {
        s = RR[pd->N];
        if (pd->go2 == 0)
            g = 0;
        else
            g = -open_penalty2(pd->A+i+1,pd->B+pd->N);
        tl -= ext_penalty2(pd->A+i+1,pd->B+pd->N);
        RR[pd->N] = hh = g+tl;
        t = open_penalty1(pd->A+i,pd->B+pd->N);
        f = RR[pd->N]-t;

        for(j=pd->N-1;j>=0;j--) {
            g = open_penalty1(pd->A+i,pd->B+j+1);
            h = ext_penalty1(pd->A+i,pd->B+j+1);
            if ((hh=hh-g-h) > (f=f-h-g+t))
                f=hh;
            t = g;
            g = open_penalty2(pd->A+i+1,pd->B+j);
            h = ext_penalty2(pd->A+i+1,pd->B+j);
            hh=RR[j]-g-h;
            if (i==(pd->M-1))
                e=SS[j]-h;
            else {
                e=SS[j]-h-g+open_penalty2(pd->A+i+2,pd->B+j);
                gS[j] = g;
            }
            if (hh > e)
                e=hh;
            hh = s + prfscore(pd->A+i+1, pd->B+j+1);
            if (f>hh)
                hh = f;
            if (e>hh)
                hh = e;
    ...
    SS[pd->N]=RR[pd->N];
    gS[pd->N] = open_penalty2(pd->A+pd->midi+1,pd->B+pd->N);

    /* take care of the barrier  */
    pthread_mutex_lock(&pdiff_mtx);
        pdiff_barrier--;
        pthread_cond_signal(&pdiff_cond);
    pthread_mutex_unlock(&pdiff_mtx);
    
    pthread_exit(NULL);
}

void pdiff_forward(pdiff_t *pd) {
    ...
    HH[0] = 0.0;
    t = -open_penalty1(pd->A,pd->B+1);
    tl = -ext_penalty1(pd->A,pd->B+1);
    for(j=1;j<=pd->N;j++) {
        HH[j] = t = t+tl;
        DD[j] = t-open_penalty2(pd->A+1,pd->B+j);
    }

    if (pd->go1 == 0)
        t = 0;
    else
        t = -open_penalty2(pd->A+1,pd->B);
    tl = -ext_penalty2(pd->A+1,pd->B);
    for(i=1;i<=pd->midi;i++) {
        s = HH[0];
        HH[0] = hh = t = t+tl;
        f = t-open_penalty1(pd->A+i,pd->B+1);

        for(j=1;j<=pd->N;j++) {
            g = open_penalty1(pd->A+i,pd->B+j);
            h = ext_penalty1(pd->A+i,pd->B+j);
            if ((hh=hh-g-h) > (f=f-h))
                f=hh;
            g = open_penalty2(pd->A+i,pd->B+j);
            h = ext_penalty2(pd->A+i,pd->B+j);
            if ((hh=HH[j]-g-h) > (e=DD[j]-h))
                e=hh;
            hh = s + prfscore(pd->A+i, pd->B+j);
            if (f>hh)
                hh = f;
            if (e>hh)
                hh = e;
...
lint pdiff(sint A,sint B,sint M,sint N,sint go1, sint go2)
{
    ...
    /* 
     *  here is a performance booster
     *  based on the speed of pthread_create() sometimes it is wiser to just
     *  go on with the original sequential way of doing things
     *  the threshold was calculated using thr_create_usec() from
     *  pth_measure.c
     */
    ...
    if (midi < threshold) {
        ...
        /*
         * following section is parallel - Solaris fixe applied, code broken up
         * into two threads - one for forward calculation, one for reverse
         */
        pthread_attr_init(&prf_attr);
        pthread_attr_setscope(&prf_attr, PTHREAD_SCOPE_SYSTEM);
        pthread_attr_setdetachstate(&prf_attr, PTHREAD_CREATE_DETACHED);
        
        pdiff_barrier = 2;  
        
        /* init memory */
        pd = malloc(sizeof(pdiff_t));
        if (!pd)
            fatal("prfalign.c:pdiff():no memory for thread data ");
        pd->A = A; pd->B = B; pd->M = M; pd->N = N;
        pd->midi = midi; pd->go1 = go1; pd->go2 = go2;
    
        res = pthread_create(&p1, &prf_attr, (void *)pdiff_forward, (void *)pd);
        if (res != 0)
            fatal(strerror(errno));
    
        res = pthread_create(&p2, &prf_attr, (void *)pdiff_reverse, (void *)pd);
        if (res != 0)
            fatal(strerror(errno));
    
        pthread_mutex_lock(&pdiff_mtx);
        while (pdiff_barrier > 0)
            pthread_cond_wait(&pdiff_cond, &pdiff_mtx);
        pthread_mutex_unlock(&pdiff_mtx);
        
        /* pthread_join(p1, NULL);
        pthread_join(p2, NULL);  */
        pthread_attr_destroy(&prf_attr);
        free(pd);
    }
