...
void fillGenomes(struct genome_struct *g, int num_genomes, int num_genes) {
  ...
  for (i=1 ; i<num_genomes ; i++) {
    ...
    for (j=num_genes-1 ; j>=1 ; j--) {
      r = (int)floor((double)(j-1) * ((double)random()/(double)MAX_RAND));
      tmp = g[i].genes[r];
...
void printGeneAdjMatrix(int **adj, int num_genes) {
  ...
  for (i=1 ; i<=num ; i++) {
    ...
    for (j=1 ; j<=num ; j++) {
      fprintf(outfile,"%3d \t", mapToPolarity(adj[i][j], num_genes));
    }
    fprintf(outfile,"\n");
  }
  fprintf(outfile,"\n");
...
void encodeAdjGenomes(int **adj, struct genome_struct *mygenomes,
  ...
  for (i=1 ; i<num_genes ; i++) {
    geneVectorG0[v] = i;
    geneVectorG1[v] = i+1;
    adj[mapToLinear(     i, num_genes)][mapToLinear(i+1, num_genes)] = 0;
    adj[mapToLinear(-(i+1), num_genes)][mapToLinear( -i, num_genes)] = 0;
    v++;
  }
  geneVectorG0[v] = num_genes;
  geneVectorG1[v] = 1;
  adj[mapToLinear(num_genes, num_genes)][mapToLinear(          1, num_genes)] = 0;
  adj[mapToLinear(       -1, num_genes)][mapToLinear( -num_genes, num_genes)] = 0;
  v++;
  ...
  for (i=1 ; i<=num ; i++)
	...
	adj[mapToLinear( i, num_genes)][mapToLinear( j, num_genes)] = 0;
	adj[mapToLinear(-j, num_genes)][mapToLinear(-i, num_genes)] = 0;
	v++;
      }
  ...
  for (i=0 ; i<v ; i++)
    fprintf(outfile,"V[%3d]: ( %4d, %4d)\n",i,geneVectorG0[i], geneVectorG1[i]);
  fprintf(outfile,"\n");
#endif
  
  for (i=0 ; i<num_genomes ; i++) {
    mygenomes[i].encoding = (char *)malloc((width+1)*sizeof(char));
...
void findAllHammingDistancesFromEncoding(struct genome_struct *mygenomes,
  ...
  for (i=0 ; i<num_genomes ; i++) {
    for (j=i+1; j<num_genomes ; j++) {
      dist = hamming_distance_from_encoding(mygenomes+i, mygenomes+j);
      fprintf(outfile,"The Hamming Distance between %s and %s is %3d (BP=%3d)\n",
	      mygenomes[i].gnamePtr, mygenomes[j].gnamePtr,
	      dist, dist/2);
    }
  }
  fprintf(outfile,"\n");
...
int hamming_distance_quad(struct genome_struct *g1, struct genome_struct *g2,
			  int num_genes) {
  int i, num;
  int gene0, gene1;
  num=0;
...
int hamming_distance_nomem(struct genome_struct *g1, struct genome_struct *g2,
...
void setBPmatrix(int **distmatrix,struct genome_struct *genomes,
  ...
  for (i=0 ; i<num_genomes ; i++) {
	...
	hamming_distance(genomes+i,genomes+j,num_genes,CIRCULAR,distmem->hammingArr);
    }
  }
}
