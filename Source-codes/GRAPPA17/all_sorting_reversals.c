...
/* Pass genome_structs for permutation "pi" and target "origin", with
   ...
   one will be created. */
void find_all_sorting_reversals(List* l, List* m, struct genome_struct* pi, 
  ...
  /* first get labels describing membership of gray edges in connected
     components */
  find_connected_components(0, n, mem->inv2, mem->upi, mem->comp_label_by_pos, 
                            NULL, -1, mem);

  /* get the component to which each cycle belongs */
  ncomp = 0;
  for (i = 0; i <= ngenes; i++) {
    mem->comp_label_by_cycle[mem->cycle[i]] = mem->comp_label_by_pos[i*2]; 
  ...
  for (i = 0; i < c; i++) {
    ...
    if (mem->conn_comp[mem->comp_label_by_cycle[i]].type != ORIENTED && 
        list_size(&mem->cyclelist[i]) > 0 && list_size(&mem->cyclelist[c+1+i]) > 0)
  ...
  nsuperhurdles = 0;
  nhurdles = nuoc;
  if (nuoc >= 3) {              /* if two or less, all must be simple
                                   hurdles */
    ...
    build_uoc_list(&uoc_list, mem->conn_comp, mem->comp_label_by_pos, n);    
  ...
  if (neutral_mode == 1 && nfortresses == 1) {
    fprintf(stderr, "ERROR: Can't handle fortresses in neutral mode.\n");
    assert(0);
  }
  ...
  for (i = 0; i < ncomp; i++) {
    ...
    if (list_size(&mem->conn_comp[i].double_superhurdle_partners) > 0) {
      printf(" (double superhurdle with %d", 
             (int)list_get(&mem->conn_comp[i].double_superhurdle_partners, 0));
      if (list_size(&mem->conn_comp[i].double_superhurdle_partners) > 1)
        printf(" and %d", (int)list_get(&mem->conn_comp[i].
                                        double_superhurdle_partners, 1));
      printf(")");
    }
...
#ifdef BITWISE_DETECT
        nhf = new_nhurdles_plus_nfortresses(mem->comp_label_by_cycle[i], 
                                            rev, n, ncomp, nuoc, nhurdles, 
                                            nfortresses, mem);
#else
        nhf = new_nhurdles_plus_nfortresses_cc(mem->comp_label_by_cycle[i], 
                                               rev, n, ncomp, nuoc, nhurdles, 
                                               nfortresses, mem) ;
#endif
  ...
  if (neutral_mode) {
    for (i = 0; i <= c; i++) {
      int i2;
      for (i2 = 0; i2 < 2; i2++) {
        for (j = 0; j < list_size(&mem->cyclelist[i2*(c+1) + i]); j++) {
          for (k = j + 1; k < list_size(&mem->cyclelist[i2*(c+1) + i]); k++) {
            int a, b, nhf;
            rev = (Reversal*)malloc(sizeof(Reversal));
            a = (int)list_get(&mem->cyclelist[i2*(c+1) + i], j);
            b = (int)list_get(&mem->cyclelist[i2*(c+1) + i], k);
            rev->start = (a <= b ? a : b);
            rev->stop = (a <= b ? b : a);
            ...
            else if (mem->conn_comp[mem->comp_label_by_cycle[i]].type != SIMPLEHURDLE) 
    ...
    if (nfortresses == 1 || nsuperhurdles < 3 || nsuperhurdles % 2 == 0 || 
      ...
      for (i = 0; i < ncomp; i++) {
        if (mem->conn_comp[i].type == SIMPLEHURDLE)
          add_all_cutting_reversals(l, &mem->conn_comp[i], mem->cyclelist);
        else if (nfortresses == 1 && mem->conn_comp[i].type == PSEUDOHURDLE) {
          npseudohurdles++;
          add_all_cutting_reversals(l, &mem->conn_comp[i], mem->cyclelist);
          add_all_cutting_reversals(l, &mem->conn_comp[mem->conn_comp[i].
    ...
    if (nfortresses == 1 || nsuperhurdles < 3 || nsuperhurdles % 2 == 0 || 
      ...
      for (i = 0; i < ncomp; i++) {
        if (mem->conn_comp[i].type == SIMPLEHURDLE) {
          ...
          Reversal *rev;
          for (j = 0; j < list_size(&mem->conn_comp[i].cyclelist); j++) {
            for (j2 = j+1; j2 < list_size(&mem->conn_comp[i].cyclelist); j2++) {
              edges1 = &mem->cyclelist[(int)list_get(&mem->conn_comp[i].cyclelist, j)];
              edges2 = &mem->cyclelist[(int)list_get(&mem->conn_comp[i].cyclelist, j2)];
  ...
  if (nhurdles >= 2) {
    ...
    for (i = 0; i < list_size(mem->mhurdles); i++) {
      for (j = i+1; j < list_size(mem->mhurdles); j++) {
        ...
        if (nfortresses == 0 && 
            eliminating_components_creates_fortress(h1, h2, mem->conn_comp, 
                                                    nhurdles, nsuperhurdles))
          continue;
        ...
        add_all_merging_reversals(l, &mem->conn_comp[h1], 
                                  &mem->conn_comp[h2], mem->cyclelist, c);
  ...
  if (neutral_mode || nhurdles >= 2) {
    ...
    find_separating_hurdles(is_separator, sepclass, bc_hurdchain, 
                            &nseparators, n, ncomp, mem, &ocs, lidx, llabel, 
                            is_separated, neutral_mode, nfortresses);
    ...
    if (neutral_mode || nseparators > 0 || nfortresses > 0) { 
      ...
      for (i = 0; i < nseparators; i++) {
        ...
        for (k = 0; k < list_size(&ocs[i]); k++) {
          ...
          for (j = 0; j < list_size(mem->mhurdles); j++) {
            ...
            if (h == separator) continue;

            if (nfortresses == 0 && 
                eliminating_components_creates_fortress(h, separator, 
                                                        mem->conn_comp, 
                                                        nhurdles, 
                                                        nsuperhurdles))
              continue;
    
            if (form_double_superhurdle(h, separator, mem->conn_comp) &&
                nfortresses == 0) 
              continue;         /* avoid eliminating a double superhurdle */

            add_all_merging_reversals(l, &mem->conn_comp[oc], 
                                      &mem->conn_comp[h], mem->cyclelist, c);
          }
        }
      }
        ...
        for (j = i + 1; j < nseparators; j++) {
          ...
          for (k = 0; k < list_size(&ocs[i]); k++) {
            ...
            for (k2 = 0; k2 < list_size(&ocs[j]); k2++) {
              ...
              add_all_merging_reversals(l, &mem->conn_comp[o1], 
                                        &mem->conn_comp[o2], mem->cyclelist, c);              
            }
          }
        }
      }
      ...
      if (neutral_mode || nfortresses == 1) {
        ...
        for (i = 0; i < ncomp; i++)
          if (is_separated[i] == 0 && (mem->conn_comp[i].type == ORIENTED || 
                                       mem->conn_comp[i].type == TRIVIAL))
        ...
        if (nfortresses == 1) {
          ...
          for (i = 0; i < list_size(mem->mhurdles); i++) {
            ...
            for (j = 0; lidx[h] != -1 && j < list_size(&ocs[lidx[h]]); j++) {
            ...
            if (mem->conn_comp[h].anchor != -1) {
              push(&to_merge2, (void*)mem->conn_comp[h].anchor);
            }
            ...
            for (j = 0; j < ncomp; j++) {
              if ((mem->conn_comp[j].type == PROTNHURDLE ||
                  ...
                  mem->conn_comp[j].chain != mem->conn_comp[h].chain) {
            ...
            /* merge with benign components that do not have
               separating hurdles */
            for (j = 0; j < list_size(&non_separated_ocs); j++) {
              int bc = (int)list_get(&non_separated_ocs, j);
            ...
            for (j = 0; j < list_size(&to_merge1); j++) {
              ...
              for (k = 0; k < list_size(&to_merge2); k++) {
                ...
                add_all_merging_reversals(l, &mem->conn_comp[c1],
                                          &mem->conn_comp[c2], 
                                          mem->cyclelist, c);
              }
            }
        ...
        if (neutral_mode) {
          ...
          for (i = 0; i < list_size(mem->mhurdles); i++) {
            ...
            if (mem->conn_comp[hrdl].anchor != -1) 
              push(&tmpl, (void*)mem->conn_comp[hrdl].anchor);
              ...
              if (mem->conn_comp[j].type == PROTNHURDLE ||
                ...
                if (mem->conn_comp[hrdl].anchor == -1) {
                  ...
                  if (mem->conn_comp[j].chain < 0 && 
                      mem->conn_comp[j].chain != -1 * hrdl - 1)
                    push(&tmpl, (void*)j);
                }
                /* anchored case: merge with any PNH of another chain */
                else {
                  if (mem->conn_comp[j].chain != 
                      mem->conn_comp[hrdl].chain && j != mem->conn_comp[hrdl].anchor) 
                  push(&tmpl, (void*)j);
                }
              }
            }
            ...
            for (j = 0; j < list_size(&non_separated_ocs); j++) {
              ...
              if ((mem->conn_comp[hrdl].anchor != -1 && 
                   bc_hurdchain[bc] != mem->conn_comp[hrdl].chain) || 
                  (mem->conn_comp[hrdl].anchor == -1 && 
                   bc_hurdchain[bc] < 0 && bc_hurdchain[bc] != -1 * hrdl -1))
                push(&tmpl, (void*)bc);
            }
            /* double superhurdle partners */
            for (j = 0; 
                 j < list_size(&mem->conn_comp[hrdl].double_superhurdle_partners);
                 j++)
              if ((int)list_get(&mem->conn_comp[hrdl].
                                double_superhurdle_partners, j) > hrdl) {
                                /* '>' to be sure we only add once */ 
                int partner = (int) list_get(&mem->conn_comp[hrdl].
                                             double_superhurdle_partners, j);
                push(&tmpl, (void*)partner);
                
                /* also add separated benign components */
                for (k = 0; lidx[partner] != -1 && 
                       k < list_size(&ocs[lidx[partner]]); k++) 
                  push(&tmpl, list_get(&ocs[lidx[partner]], k));
              }
            ...
            for (j = 0; j < list_size(&tmpl); j++) {
              ...
              if (hrdl != comp)
                add_all_merging_reversals(m, &mem->conn_comp[hrdl], 
                                          &mem->conn_comp[comp], mem->cyclelist, 
                                          c);

              if (lidx[hrdl] != -1) {
                for (k = 0; k < list_size(&ocs[lidx[hrdl]]); k++) {
                  int c1 = (int)list_get(&ocs[lidx[hrdl]], k);
                  add_all_merging_reversals(m, &mem->conn_comp[c1], 
                                            &mem->conn_comp[comp], 
                                            mem->cyclelist, c);
                }
              }
            ...
            if (lidx[hrdl] != -1 && mem->conn_comp[hrdl].type == SIMPLEHURDLE) {
              for (k = 0; k < list_size(&ocs[lidx[hrdl]]); k++) {
                int k2, c1 = (int)list_get(&ocs[lidx[hrdl]], k);
                for (k2 = k+1; k2 < list_size(&ocs[lidx[hrdl]]); k2++) {
                  int c2 = (int)list_get(&ocs[lidx[hrdl]], k2);
      ...
      for (i = 0; i < nseparators; i++) free_list(&ocs[i]);
      free(ocs);
    }
  }
...
void find_connected_components(int begidx, int len, int *inv2, int *upi,
  ...
  for (i = begidx; i < len + begidx; i++) {
    ...
    while (mem->cc_beg[top] > mem->cc_beg[mem->cc_e[i]]) {
      extent_b = (extent_b < mem->cc_beg[top] ? extent_b : mem->cc_beg[top]);
      extent_e = (extent_e > mem->cc_end[top] ? extent_e : mem->cc_end[top]);
    ...
    if (mem->cc_parent[dest_comp_label[i]] == -1) {
      if (i == dest_comp_label[i]) {
        ...
        if (restrict_to == -1)  /* don't do this in the "detect" case */
          mem->conn_comp[comp].begidx = mem->conn_comp[comp].endidx = i;
        comp++;
      }
      else {                    /* refer to label at beginning of this
                                   component*/
        dest_comp_label[i] = 
          dest_comp_label[dest_comp_label[i]];
        if (restrict_to == -1)  /* don't do this in the "detect" case */
          mem->conn_comp[dest_comp_label[i]].endidx = i + 1;
      }
    }
    else {                      /* use label of parent */
      dest_comp_label[i] = 
...
void build_uoc_list(List *uoc_list, ConnectedComponent *conn_comp, 
  ...
  for (i = 0; i < n; i++) {
    ...
    if (conn_comp[current].type != ORIENTED && 
        conn_comp[current].type != TRIVIAL && 
        current != prev) {
...
void find_superhurdles(List *uoc_list, ConnectedComponent *conn_comp, 
  ...
  j = 0;
  label[j] = prevlbl; idx[prevlbl] = j; j++;
  for (i = 0; i < list_size(uoc_list); i++) {
    int currentlbl;
    currentlbl = (int)list_get(uoc_list, i);
    if (idx[currentlbl] == -1) {
      label[j] = currentlbl; idx[currentlbl] = j; j++;
    }
    if (prevlbl != currentlbl && 
        ! list_contains(&adj_list[idx[prevlbl]], (void*)(idx[currentlbl]))) {
  ...
  for (i = 0; i < nuoc; i++) {
    mark[i] = 0;
    pred[i] = -1;
    init_list(&cycles[i], nuoc/2, sizeof(int)); 
  }
  ncycles = 0;
  push(&S, 0);
  while (!empty(&S)) {
  ...
  *nsuperhurdles = 0;
  for (i = 0; i < nuoc; i++) {
    ...
    if (on_cycles > 0) {
      if (deg >= 3) {
        ...
        if (deg > 3) conn_comp[label[i]].mult_protected = 1;
        conn_comp[label[i]].chain = -1;
      }
      else {                    /* degree of exactly 2 */
      ...
      if (deg >= 2) {
        conn_comp[label[i]].type = PROTNHURDLE;
        if (deg > 2) conn_comp[label[i]].mult_protected = 1;
        conn_comp[label[i]].chain = -1;
      }
      else {                    /* degree of exactly one */
        int n, degn, cyclesn;
  ...
  for (i = 0; i < list_size(&three_cycles_memb); i++) {
    ...
    for (j = 0; j < 3; j++) {
      ...
      if (deg[j] == 3 || (deg[j] == 4 && list_size(&cycles[u[j]]) == 2)) {
        ...
        if (deg[k] == 2) v = u[k];
        else v = find_single_hurdle(u[k], adj_list, cycles);
        if (deg[l] == 2) w = u[l];
        else w = find_single_hurdle(u[l], adj_list, cycles);
        if (v != -1 && w != -1) {
        ...
        if (list_size(&adj_list[i]) == 1) {
          ...
          while (!stop) {
            ...
            if (list_size(&adj_list[j]) > 2) {
              ...
              int x1, x2;
              conn_comp[label[i]].anchor = conn_comp[label[j]].anchor = -1; 
                                /* unanchored */
              stop = 1;
            ...
            else {              /* degree exactly two (middle of chain) */
...
int find_single_hurdle(int u, List *adj_list, List *cycles) {
  ...
  while (1) {
    ...
    if (deg > 2) return -1;
    if (deg == 1) return v;     /* we've reached the end of the line */
...
void build_overlap_matrix(ConnectedComponent *cc, int *upi, int *inv2, 
  ...
  for (i = 0; i < list_size(&cc->cyclelist); i++) {
    ...
    for (t = 0; t <= 1; t++) {  /* consider edges of each orientation */
      List *belist;
      belist = &cyclelist[t * (c + 1) + cycno];
      for (j = 0; j < list_size(belist); j++) {
        for (k = 0; k <= 1; k++) { /* consider both sides of black edge */
          idx = (int)list_get(belist, j) * 2 + k;
          if (upi[idx] % 2 == 0) { /* consider each gray edge only when we
                                      find its even-numbered end (to
                                      avoid counting twice) */
            push(&cc->beg, (void*)(idx < inv2[upi[idx]+1] ? idx : inv2[upi[idx]+1]));
            push(&cc->end, (void*)(idx < inv2[upi[idx]+1] ? inv2[upi[idx]+1] : idx));
...
void detect_new_unoriented_components(Reversal *rho, 
  ...
  for (i = 0; i < list_size(&cc->grey_edges); i++) {
    printf("v(%d) [%d,%d]:\t", i, (int)list_get(&cc->grey_edges, i), (int)list_get(&cc->grey_edges,i)+1);
...
void count_hurdles_after_reversal(List *V, int* comp_label_by_position, 
  ...
  int new_nuoc, i, j, newcomp, label, startpos, stoppos, 
    new_nsuperhurdles;
  List new_uoc_list;
  ...
  for (i = 0; i < n; i++) {
    if (i < startpos || i >= stoppos) 
      ...
      comp_label_copy[i] = comp_label_by_position[startpos + stoppos -1 - i];
  ...
  *new_nhurdles = new_nsuperhurdles = 0;
  for (i = 0; i < ncomp; i++) {
    if (conn_comp[i].type == SIMPLEHURDLE || 
        conn_comp[i].type == PROTNHURDLE || conn_comp[i].type == SUPHURDLE ||
    ...
    init_list(&new_conn_comp[i].double_superhurdle_partners, 2, sizeof(int));
...
int new_nhurdles_plus_nfortresses_cc(int ccidx, Reversal *rev, 
  ...
  copy_for_reversal(rev, cc->begidx, cc->endidx - cc->begidx, mem->inv2_cpy, 
                    mem->upi_cpy, mem->comp_label_cpy, mem->be_cpy, mem);

  find_connected_components(cc->begidx, cc->endidx - cc->begidx, 
                            mem->inv2_cpy, mem->upi_cpy, mem->new_comp_labels, 
                            mem->comp_label_cpy, ccidx, mem);
  ...
  for (i = cc->begidx; i < cc->endidx; i++) {
    if (mem->comp_label_cpy[i] != ccidx) continue;
    ...
    if (mem->comp_mark[comp] == 0 && i % 2 == 0 && mem->be_cpy[i/2] == 1)
...
void add_all_merging_reversals(List *l, ConnectedComponent *c1, 
  ...
  for (i = 0; i < list_size(&c1->cyclelist); i++) {
    for (j = 0; j < list_size(&c2->cyclelist); j++) {
      for (o1 = 0; o1 < 2; o1++) {
        for (o2 = 0; o2 < 2; o2++) {
          edges1 = &cyclelist[o1 * (c + 1) + (int)list_get(&c1->cyclelist, i)];
          edges2 = &cyclelist[o2 * (c + 1) + (int)list_get(&c2->cyclelist, j)];
          for (k = 0; k < list_size(edges1); k++) {
            for (k2 = 0; k2 < list_size(edges2); k2++) {
              a = (int)list_get(edges1, k);
              b = (int)list_get(edges2, k2);
...
int form_double_superhurdle(int h1, int h2, ConnectedComponent *conn_comp) {

  if ((list_size(&conn_comp[h1].double_superhurdle_partners) >= 1 && 
       (int)list_get(&conn_comp[h1].double_superhurdle_partners, 0) == h2) ||
      (list_size(&conn_comp[h1].double_superhurdle_partners) == 2 && 
       (int)list_get(&conn_comp[h1].double_superhurdle_partners, 1) == h2))
    return 1;
...
ReversalSortingMemory* new_reversal_sorting_memory(int ngenes) {
  ...
  mem = (ReversalSortingMemory*)malloc(sizeof(ReversalSortingMemory));
  mem->ngenes = ngenes;
  ...
  mem->comp_label_by_cycle = (int*)malloc((ngenes + 1) * sizeof(int));
  mem->cyclelist = (List*)malloc(2 * (ngenes + 2) * sizeof(List));
  mem->conn_comp = (ConnectedComponent*)malloc((ngenes + 1) * 
  ...
  for (i = 0; i < ngenes + 1; i++) {
    init_list(&mem->conn_comp[i].cyclelist, ngenes + 1, sizeof(int));
    init_list(&mem->conn_comp[i].double_superhurdle_partners, 2, sizeof(int));
  }
  ...
  mem->bv_v = (BitVector*)malloc((ngenes + 1) * sizeof(BitVector));
  for (i = 0; i < ngenes + 1; i++)
  ...
  for (i = 0; i < ngenes + 1; i++) {
    ...
    mem->conn_comp[i].v = (BitVector*)malloc((ngenes + 1) * sizeof(BitVector));
  ...
  mem->comp_grey_edges = (List*)malloc((n/4 +1) * sizeof(List));
  for (i = 0; i < n/4 + 1; i++)
...
void copy_for_reversal(Reversal *rev, int begidx, int len, int *inv2_cpy, 
  ...
  for (i = begidx; i < begidx + len; i++) {
    if (i < startpos || i >= stoppos) {
      ...
      comp_label_cpy[i] = mem->comp_label_by_pos[startpos + stoppos -1 - i];
...
void find_separating_hurdles(int *is_separator, int *sepclass, 
  ...
  for (i = 0; i < ncomp; i++) { is_separator[i] = 0; sepclass[i] = -1; }
  ...
  for (i = 0; i < ncomp; i++) bc_hurdchain[i] = ncomp;
  ...
  for (i = start + 1; i <= start + n; i++) {
    ...
    if (comp == currenth) { 
      ...
      while (! empty(&tmpl)) {
        ...
        else if (prevuoc == currentuoc && mem->conn_comp[prevuoc].chain < 0)
        ...
        /* otherwise if the UOCs on either side of the BC belong to
           ...
           and for anchors of chains that are "multiply protected") */
                 ...
                 (prevuoc != currentuoc || 
                  prevuoc != mem->conn_comp[mem->conn_comp[prevuoc].chain].
                  anchor)) {
          ...
          if ((mem->conn_comp[prevuoc].chain == 
            ...
            bc_hurdchain[c] = mem->conn_comp[prevuoc].chain;
              ...
              mem->conn_comp[comp].type == TRIVIAL) && mark[comp] == 0) {
  ...
  if (neutral_mode || *nseparators > 0 || nfortresses > 0) { 
    ...
    *ocs = (List*)malloc(*nseparators * sizeof(List));
    ...
    for (i = 0; i < list_size(&separated_ocs); i++) {
      ...
      j = lidx[h];
      if (j == -1) { j = counter++; lidx[h] = j; llabel[j] = h; }
      push(&(*ocs)[j], (void*)oc);
    }
  }
