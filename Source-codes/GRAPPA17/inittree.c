...
void initialize_tree_random(struct tNode *tree, struct genome_struct *labels,
...
void initialize_tree_trivial(struct tNode *tree, struct genome_struct *labels,
                             int num_genes,int num_genomes) {
...
INLINE struct tNode *dequeue(struct qNode *queue,
			     struct qNode **head,
			     struct qNode **tail) {
  struct tNode *node;
#ifdef DEBUG
...
INLINE void add_queue(struct qNode *queue,
		      struct tNode *node,
		      struct qNode **head,
		      struct qNode **tail) {
  (*tail)->item = node;
...
struct tNode *find_tree_middle(struct tNode *tree,struct qNode *queue,
...
#ifdef DEBUG
  if (tree->tag < 0) {
    fprintf(outfile,"Error in find_tree_middle: start vertex is internal\n");
    fflush(outfile);
  }
#endif
  head = tail = queue; /* initialize queue pointers */
  for (i = 0; i <= 2*num_genomes; i++) {  /* set all flags to false */
    tpool[i].leaf = FALSE;
  }
  node = tree;
  node->leaf = TRUE;
  ...
  while  (head != tail) {
    ...
    if (node->tag < 0) { /* only internal nodes have children */
      next = node->lChild;
      if (next->leaf == FALSE) {
	next->leaf = TRUE;
	next->pred = node;
...
void combine(int *gene, struct genome_struct *g1, struct genome_struct *g2,
             int num_genes) {
  int i;
#ifdef DEBUG
...
void propagate(struct tNode *current,struct genome_struct *labels,int num_genes,
               int num_genomes,int *succ1,int *succ2f,int *succ2b,int *degree,
               int *pred1,int *pred2,int *tour,int *otherEnd,
               int comb_method) {
  ...
  if (parent) {
    if (!(parent->leaf)) {
      propagate(parent,labels,num_genes,num_genomes,
		succ1,succ2f,succ2b,degree,pred1,pred2,tour,otherEnd,
		comb_method);
    }
  }
  ...
  if (parent->genome == NULL) {
    if (comb_method==0) { /* FASTPROP */
      combine(gen->genes,lChild->genome,rChild->genome,num_genes);
    }
    else { /* MEDIANPROP */
      greedy_median(gen->genes,lChild->genome,rChild->genome,num_genes,
		    degree,succ1,succ2f,succ2b,pred1,pred2,tour,otherEnd);
    }
  }
  else {
    if (lChild->genome == NULL) {
      if (comb_method==0) { /* FASTPROP */
	combine(gen->genes,parent->genome,rChild->genome,num_genes);
      }
      else { /* MEDIANPROP */
	greedy_median(gen->genes,parent->genome,rChild->genome,num_genes,
		      degree,succ1,succ2f,succ2b,pred1,pred2,tour,otherEnd);
      }
    }
    else { /* by default, rChild->genome must be null */
      if (comb_method==0) { /* FASTPROP */
	combine(gen->genes,parent->genome,lChild->genome,num_genes);
      }
      else { /* MEDIANPROP */
	greedy_median(gen->genes,parent->genome,lChild->genome,num_genes,
		      degree,succ1,succ2f,succ2b,pred1,pred2,tour,otherEnd);
      }
    }
  }
...
void initialize_tree_propagate(int COND,struct tNode *tree,struct tNode *tpool,
  ...
  if (parent->tag < 0) {
    propagate(parent,labels,num_genes,num_genomes,
	      ...
	      pred1,pred2,outcycle,otherEnd,
	      comb_method);
  }
  lChild = root->lChild;
  if (lChild->tag < 0) {
    propagate(lChild,labels,num_genes,num_genomes,
	      stack,picked,incycle,degree,pred1,pred2,outcycle,otherEnd,
	      comb_method);
  }
  rChild = root->rChild;
  if (rChild->tag < 0) {
  ...
  if (COND) { /* condensation */
     ...
     condense3(parent->genome->genes,
	       ...
	       pred1,pred2,picked,
#else
	       pred1+num_genes, pred2+num_genes,
	       picked+num_genes, 
#endif
	       decode);
     ...
     if ((CIRCULAR && (num_cond==0)) || ((!CIRCULAR) && (num_cond<=1)))  {
	...
	goto done; /* and exit */
     ...
     case TSP_BBTSP:
        /* does not create a weight matrix, but sets weight in the adj lists */
        convert2_to_tsp(parent->genome,lChild->genome,rChild->genome,
  		        adj_list,adj_pool,num_genes,CIRCULAR);
        bbtsp(2*num_genes,root->genome->genes,FALSE,
              parent->genome->genes,lChild->genome->genes,rChild->genome->genes,
  	      adj_list,neighbors,stack,outcycle,degree,otherEnd,edges,
	      CIRCULAR);
        break;
#ifdef CONCORDE
     case TSP_CHLINKERN:
        /* creates a weight matrix -- more expensive! */
        convert_to_tsp(parent->genome,lChild->genome,rChild->genome,
...
struct tNode *find_closest_leaf(struct tNode *node,struct qNode *queue) {
  ...
  head = tail = queue; /* initialize queue pointers */
  node->leaf = TRUE;
  while  (1) { /* loop until a leaf is found -- one must exist */
    if (node->tag >= 0) {
      return node;
    }
    else { /* can move in 2 of 3 directions, but which? */
      next = node->parent;
      if (next->leaf == FALSE) {
	next->leaf = TRUE;
	add_queue(queue,next,&head,&tail);
...
void real_init_tree_NN(int COND,struct tNode *tree,struct tNode *tpool,
  ...
  if (tree->tag >= 0) return; /* nothing to do at a leaf */
  ...
  /* now take the median of these three leaves */
#ifdef DEBUG
  fprintf(outfile,"in real_init_NN, about to convert, id=%3d, A=%3d, B=%3d, C=%3d\n",
  ...
  if (COND) { /* condensation */
    ...
    if (num_cond==0)  { /* three identical neighbors... */
      ...
      for (i=0; i<num_genes; i++) { /* so just copy one in place */
	nodem->genes[i] = node1->genes[i];
      }
    }
    else { /* get the median the hard way ;-) */
	...
	case TSP_COALESCED:
          convert2_to_tsp(condnode1->genome,condnode2->genome,condnode3->genome,
		          adj_list,adj_pool,num_cond,CIRCULAR);
          coalestsp(2*num_cond,condmedian->genome->genes,FALSE,
	        condnode1->genome->genes,condnode2->genome->genes,
	        condnode3->genome->genes,
	        adj_list,neighbors,stack,outcycle,degree,otherEnd,edges,
	        CIRCULAR);
      ...
      decode3(nodem->genes,condmedian->genome->genes,succ,decode,num_cond);
    }
  } /* end condensation */
  else { /* no condensation */
    convert2_to_tsp(node1,node2,node3,adj_list,adj_pool,num_genes,CIRCULAR);
    /* needs a small fix here: bbtsp will try to use existing
       genome at tree->genome as a tour, which will crash, since
       there is not one yet...  so pass FALSE to use_median */
    realsolver = inittspsolver;
    ...
    switch (realsolver) {
      ...
      case INVERSION_MEDIAN_FAST:
        ...
        gen[2]=node3;
        if (CIRCULAR)
            albert_inversion_median_circular(gen, num_genes, nodem->genes);
        else
            albert_inversion_median_noncircular(gen, num_genes, nodem->genes);
        break;
      case TSP_COALESCED:
  ...
  if (tree->lChild)
    real_init_tree_NN(COND,tree->lChild,tpool,labels,triple,adj_list,adj_pool,
		      weights,stack,degree,otherEnd,neighbors,edges,incycle,
		      outcycle,pred,succ,code,decode,condnode1,condnode2,
		      condnode3,condmedian,num_genes,num_genomes,inittspsolver,
		      thresh,CIRCULAR);
  if (tree->rChild)
    real_init_tree_NN(COND,tree->rChild,tpool,labels,triple,adj_list,adj_pool,
		      weights,stack,degree,otherEnd,neighbors,edges,incycle,
		      outcycle,pred,succ,code,decode,condnode1,condnode2,
		      condnode3,condmedian,num_genes,num_genomes,inittspsolver,
		      thresh,CIRCULAR);
...
int move_up(struct tNode *current,triple_t *triple,int num_genomes) {
...
void move_down(struct tNode *parent,struct tNode *current,int which,
  ...
  id = parent->tag;
  if (id >= 0) { /* parent is a leaf */
    triple[currind].C = id;
  }
  else { /* parent is an internal node */
    switch (which) {
...
void initialize_tree_BNN(int COND,struct tNode *tree,struct tNode *tpool,
  ...
  if (tree->lChild != NULL) move_up(tree->lChild,triple,num_genomes);
  if (tree->rChild != NULL) move_up(tree->rChild,triple,num_genomes);

  if (tree->lChild != NULL) move_down(tree,tree->lChild,0,triple,num_genomes);
  if (tree->rChild != NULL) move_down(tree,tree->rChild,1,triple,num_genomes);
#ifdef DEBUG
  ...
  real_init_tree_NN(COND,tree->lChild,tpool,labels,triple,adj_list,adj_pool,
		    weights,stack,degree,otherEnd,neighbors,edges,incycle,
		    outcycle,pred+num_genes,succ+num_genes,code+num_genes,
		    decode,condnode1,condnode2,condnode3,
		    condmedian,num_genes,num_genomes,inittspsolver,
		    thresh,CIRCULAR);
  return;
}

/* used by NN version for small numbers of genomes -- quadratic work */
void realinit_tree_SNN(int COND,struct tNode *tree,struct tNode *tpool,
                       struct genome_struct *labels,struct qNode *queue,
                       struct adj_struct *adj_list,struct adj_struct *adj_pool,
                       int **weights,int*stack,int *degree,int *otherEnd,
                       intpair_t *neighbors,edge_t *edges,
  ...
  if (tree->tag < 0) { /* internal node needs to be processed */
	...
	convert2_to_tsp(condnode1->genome,condnode2->genome,
			condnode3->genome,adj_list,adj_pool,num_cond,CIRCULAR);
	/* needs a small fix here: bbtsp will try to use existing
	 * genome at tree->genome as a tour, which will crash, since
	 * there is not one yet...  so pass FALSE to use_median */
        realsolver = inittspsolver;
        ...
        case INVERSION_MEDIAN_FAST:
          ...
          case TSP_GREEDYLK:
             ...
             convert_to_tsp(condnode1->genome,condnode2->genome,
			    condnode3->genome,num_cond,CIRCULAR,weights);
             greedylk(2*num_cond,weights,condmedian->genome->genes,
  		      incycle,outcycle);
            break;
          case TSP_CHLINKERN:
  ...
  /* go process remaining nodes, if any */
  if (tree->lChild)
    realinit_tree_SNN(COND,tree->lChild,tpool,labels,queue,adj_list,adj_pool,
		      weights,stack,degree,otherEnd,neighbors,edges,incycle,
		      outcycle,pred,succ,code,decode,condnode1,condnode2,
		      condnode3,condmedian,num_genes,num_genomes,inittspsolver,
		      thresh,CIRCULAR);
  if (tree->rChild)
    realinit_tree_SNN(COND,tree->rChild,tpool,labels,queue,adj_list,adj_pool,
		      weights,stack,degree,otherEnd,neighbors,edges,incycle,
		      outcycle,pred,succ,code,decode,condnode1,condnode2,
		      condnode3, condmedian,num_genes,num_genomes,inittspsolver,
		      thresh,CIRCULAR);
...
struct tNode *find_closest_label(struct tNode *node,struct qNode *queue) {
  /* by "closest" we mean "the fewest edges away", since we
     have no values for edge lengths; thus we need a simple bfs */
  /* use node->leaf as a visited flag -- needed, since we have
     an unrooted tree */
  ...
  node->leaf = TRUE;
  while  (1) { /* loop until a leaf is found -- one must exist */
    if (node->genome != NULL) { /* node already labeled, found */
      return node;
    }
    else { /* can move in 2 of 3 directions, but which? */
      next = node->parent;
...
void realinit_tree_SBNN(int COND,struct tNode *tree,struct tNode *tpool,
                        struct genome_struct *labels,struct qNode *queue,
                        struct adj_struct *adj_list,struct adj_struct *adj_pool,
			int **weights,int*stack,int *degree,int *otherEnd,
                        intpair_t *neighbors,edge_t *edges,
...
#define SBNNpreorder
  ...
  /* go process remaining nodes, if any */
  if (tree->lChild)
    realinit_tree_SBNN(COND,tree->lChild,tpool,labels,queue,adj_list,adj_pool,
		      weights,stack,degree,otherEnd,neighbors,edges,incycle,
		      outcycle,pred,succ,code,decode,condnode1,condnode2,
		      condnode3,condmedian,num_genes,num_genomes,inittspsolver,
		      thresh,CIRCULAR);
  if (tree->rChild)
    realinit_tree_SBNN(COND,tree->rChild,tpool,labels,queue,adj_list,adj_pool,
		      weights,stack,degree,otherEnd,neighbors,edges,incycle,
		      outcycle,pred,succ,code,decode,condnode1,condnode2,
		      condnode3,condmedian,num_genes,num_genomes,inittspsolver,
		      thresh,CIRCULAR);
