/* $Id: inversion_median.c,v 1.18 2001/12/20 03:17:10 acs Exp $
...
void find_inversion_median(struct genome_struct *median, 
...
#ifdef THREADSAFE
  int t, retval;
  pthread_t* tn = (pthread_t*)calloc(NTHREADGROUPS, sizeof(pthread_t));
#endif
  ...
  sv->bestmed = (int*)calloc(sv->ngenes, sizeof(int));
...
/*   printf("%d\n", sv->MIN_MED); */
  ...
  sv->h = new_hashtable(sv->ngenes, HASH_EXPECTED_SIZE, HASH_LOADING_FACTOR);

  ht_insert(sv->h, v->perm);

  sv->ps = new_ps(sv->MIN_MED, sv->MAX_MED, QUEUECAPACITY, sizeof(Vertex*));
  ...
  for (t = 0; t < NTHREADGROUPS; t++) {
    ...
    /* spawn thread */
    if ((retval = pthread_create(&tn[t], NULL, &do_loop, &td[t])) != 0) {
...
void* do_loop(void* sv_void) {
  ...
  while (!sv->stop) {
    ...
    if (sv->stop == 1) break; 	/* in case set immediately above */
...
void* worker_thread_loop(void* vp) {
  ...
  while (!td->sv->stop) {
    ...
    /* process neighbors */
    if (td->sv->stop == 0) {	/* need to double check, because we
...
/*       find_circular_identity(n->perm, id, sv->ngenes); */
	...
	return_vertex(sv->vf, n);
      ...
      else {		/* previously unmarked */
	sv->nneighbors++;

	/* find distances to vertices */
	g.genes = n->perm;
	n->distance = invdist_noncircular(&g, sv->origin, 0, sv->ngenes, td->distmem);
	...
	d1 = invdist_noncircular(&g, sv->other1, 0, sv->ngenes, td->distmem);
	d2 = invdist_noncircular(&g, sv->other2, 0, sv->ngenes, td->distmem);

	n->best_possible_score = n->distance + ceil((d1 + d2 + sv->distbetween)/2.0);
	...
	if (n->worst_possible_score == sv->MIN_MED) {
	  ...
	  else {		/* not feasible -- prune it away */
	    return_vertex(sv->vf, n);
	  }
	}
      }
...
Vertex* get_neighbor(SearchVars *sv, Vertex* v, int start, int end) {
  int i, mid;
  Vertex* n;
