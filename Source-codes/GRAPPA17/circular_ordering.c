...
int circular_ordering(int num_genomes,int num_genes,
	      ...
	      int *stack,int *outcycle,int *degree,int *otherEnd,edge_t *edges1,
	      int CIRCULAR)
{
  ...
  for (i=0; i<num_genomes; i++) degree[i] = 0;
  ...
  if (flag == 0) newedge = edges+numedges-1; /*no need to use +*/
  else newedge = edges;
  ...
  while ((picked < num_genomes) && (newedge < (edges+numedges))) {
     ...
     if ((degree[i]<=1) && (degree[j]<=1) &&
        ...
        if (maxweight > highscore) highscore = maxweight;
        if (maxweight < lowscore) lowscore = maxweight;
        score += maxweight;
        otherEndI = otherEnd[i];
        otherEndJ = otherEnd[j];
        otherEnd[otherEndI] = otherEndJ;
        otherEnd[otherEndJ] = otherEndI;
        if ( neighbors[i].A == -1) neighbors[i].A = j;
        else  neighbors[i].B = j;
        if ( neighbors[j].A == -1) neighbors[j].A = i;
        else  neighbors[j].B = i;
        
        degree[i] += 1;
        degree[j] += 1;
        picked++;
     }
     if (flag == 0) newedge--; /* changed to ++*/
     else newedge++;
  }
  ...
  free(sortedges);
  free(edges);
  if (flag == 0) return score + highscore;
  else return score;
}
