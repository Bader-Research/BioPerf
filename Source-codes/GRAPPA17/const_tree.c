...
main() {
  ...
  tpool = (struct tNode *)malloc((2*MAX_NUM_GENES+1)*sizeof(struct tNode));
...
int init_const(env_t *env,const char *str,ConstraintTree_T const_tree,
	       struct tNode *tpool) {
 ...
 int i;
  ...
  root = str_to_tree(env,str,const_tree,&pos,&tree_nodes,&internal,tpool);
...
int str_to_tree(env_t *env,const char *str,ConstraintTree_T tree,int *pos,
  ...
  if (str[*pos]=='(') {
    ...
    while (str[*pos]!=')') {
	  ...
	  internal,tpool);
      if(nb_child==0){ child1=cur_sub_root;}
      else {tree[pre_sub_root].brother=cur_sub_root;
      }
...
#if 0
int inc_const_tree(ConstraintTree_T const_tree,env_t env,int inc,
		   int *dif_node,int *dif_lev ) {
  int i,tmp;
 
  for (i=0 ; i<env.tree_nodes ; i++){
    tmp=inc_bin_tree(const_tree[i].NumOfChild,env.dim[i],inc,dif_lev);
    if (tmp==0) {
      *dif_node=i;  return 0;
    } 
    else if (tmp>0)
...
/*scan the const_tree from down to top*/
  for (i=0; i<env->tree_nodes; i++) {
    nb_child = const_tree[i].NumOfChild;
        if ((i==env->root) &&(nb_child>=3)) leaves=nb_child;
        else leaves=nb_child+1;
  ...
  for (j=3;j<2*n+1;j++){
	...
	parent->rChild = &tpool[*pos];
      ...
      fprintf(stdout,"exp[%d]:%d,lchild:%d,child:%d,tmp->tag:%d,\n",
	      j,exp[j],lchild,
	      child[exp[j]-2],tmp->tag);
      fflush(stdout);
#endif
      tmp->parent = parent;
      if (lchild==1) 
	parent->lChild = tmp;
      else parent->rChild = tmp;
...
int gen_bin_tree(int n, int tree_id,int *new_tree) {
  int i,j;
  ...
  if (n==3) {
    /*base case*/
    for(i=0;i<size_of_tree;i++) new_tree[i] = 0;
