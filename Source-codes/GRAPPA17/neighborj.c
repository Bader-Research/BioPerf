...
double neighborj_SN(struct genome_struct *genomes, int num_genes, int num_genomes,
  ...
  for (i=0 ; i<num_genomes ; i++) {
    for (j=i+1 ; j<num_genomes ; j++) {
      if (CIRCULAR)
	D[i][j] = (double)invdist_circular(genomes+i, genomes+j,
					   num_genes, distmem);
      else 
	D[i][j] = (double)invdist_noncircular(genomes+i, genomes+j, 0,
					      num_genes, distmem);
      Dsum += D[i][j];
#ifdef DEBUG
	...
	for (j=i+1 ; j<num_genomes ; j++) {
	  if (valid[j]) {
	    ...
	    for (k=0 ; k<num_genomes ; k++) {
	      if ((k!=i) && (k!=j) && valid[k])
		Sij += (i<k ? D[i][k] : D[k][i]) + (j<k ? D[j][k] : D[k][j]);
	    }
	    Sij /= 2.0 * ((double)num_clusters - 2.0);
	    Sij += 0.5 * D[i][j];
	    Sij += (1.0 / ((double)num_clusters - 2.0)) * (Dsum - D[i][j]);
#ifdef DEBUG
...
double neighborj_SK(struct genome_struct *genomes, int num_genes, int num_genomes,
		    distmem_t *distmem, int CIRCULAR, int verbose) {
  int i, j;
  double NJtreescore;
  int num_clusters;
...
double neighborj_SK_tree(struct genome_struct *genomes,
  ...
  maxConstSize = ((int)ceil(log10((double)num_genomes)) + 4) * num_genomes + 1;
			   ...
			   int orig_num_genes, int correction)
{
  ...
  case RAND:
    ...
    break;
  case SMALLNN:
    initialize_tree_SNN(COND,tree,tpool,labels,qpool,adj_list,adj_pool,weights,
			stack,degree,otherEnd,neighbors,edges,incycle,outcycle,
			pred1,pred2,picked,decode,num_genes,num_genomes,
			inittspsolver,thresh,CIRCULAR);
    /* reuses pred1, pred2, picked */
    break;
  case SBNN:
    initialize_tree_SBNN(COND,tree,tpool,labels,qpool,adj_list,adj_pool,weights,
			 stack,degree,otherEnd,neighbors,edges,incycle,outcycle,
			 pred1,pred2,picked,decode,num_genes,num_genomes,
			 inittspsolver,thresh,CIRCULAR);
      /* reuses pred1, pred2, picked */
      break;
  case BIGNN:
    initialize_tree_BNN(COND,tree,tpool,labels,triple,adj_list,adj_pool,weights,
			stack,degree,otherEnd,neighbors,edges,incycle,outcycle,
			pred1,pred2,picked,decode,num_genes,num_genomes,
			inittspsolver,thresh,CIRCULAR);
    /* reuses pred1, pred2, picked */
  ...
  case FASTPROP:
    initialize_tree_propagate(COND,tree,tpool,labels,qpool,adj_list,adj_pool,
			      ...
			      pred1,pred2,picked,decode,FAST,tspsolver,
			      thresh,CIRCULAR);
    break;
  case MEDIANPROP:
    initialize_tree_propagate(COND,tree,tpool,labels,qpool,adj_list,adj_pool,
  ...
  case ADJPARS:
    initialize_tree_adjpars(tree,tpool,labels,
		            stack,degree,otherEnd,neighbors,smalledges,
			    incycle,outcycle,num_genes,num_genomes,
...
int neighborj_score_one(struct genome_struct *genome_list, int num_genes,
			...
			int *stack,int *degree,int *otherEnd,intpair_t *neighbors,
			smalledge_t *smalledges,
			edge_t *edges,
			int *outcycle,int *pred1,int *pred2,int *picked,
			int *decode, int inittspsolver,
			triple_t *triple,int *incycle,int tspsolver,int distmethod,
			int thresh, int **weights, int **status,
		        env_t *const_env,
			int *genes,
			int *condense_succ, int *condense_decode,
			int orig_num_genes, int correction)
{
  ...
  invscore_d = neighborj_SK_tree(genome_list, num_genes, num_genomes, distmem,
				 CIRCULAR, constTree);
  ...
  score = neighborj_score_string(constTree,
				 ...
				 thresh, weights, status, &best_so_far,
				 const_env, genes,
			...
			int orig_num_genes, int correction)
{
...
#ifdef DEBUG
  fprintf(outfile,"neighborj_SK_trees(): best_so_far: %12d  num_clust: %3d\n",
  ...
  if (num_clusters == 2) {
    ...
    sprintf(constTree,"(%s,%s)",orig_branch[mini],orig_branch[minj]);
  ...
  for (i=0 ; i<num_genomes ; i++) {
    if (valid[i]) {
      for (j=i+1 ; j<num_genomes ; j++) {
	if (valid[j]) {
	  if (fabs(minSij - ( orig_D[i][j] - (r[i] + r[j]) )) < SMALLDELTA) {
	    /* Merge these and recurse */
	
	    strcpy(tmpbranch, branch[i]);
	    sprintf(branch[i],"(%s,%s)",tmpbranch,branch[j]);
	    ...
	    for (k = 0 ; k<i ; k++) {
	      if (valid[k]) {
		if (j < k)
		  D[k][i] = 0.5 * (D[k][i] + D[j][k] - Dij);
		else
		  D[k][i] = 0.5 * (D[k][i] + D[k][j] - Dij);
	      }
	    }
	    for (k = i+1 ; k<num_genomes ; k++) {
	      if (valid[k]) {
		if (j < k)
		  D[i][k] = 0.5 * (D[i][k] + D[j][k] - Dij);
		else
		  D[i][k] = 0.5 * (D[i][k] + D[k][j] - Dij);
	      }
	    }
	    ...
	    neighborj_SK_trees(genome_list, num_genes, num_genomes,
			       ...
			       num_clusters-1, D, valid, branch,
			       maxConstSize, best_so_far, count,
			       const_env, genes,
			       condense_succ, condense_decode,
			       orig_num_genes, correction);
		    ...
		    int orig_num_genes, int correction)
{
  ...
  for (i=0 ; i<num_genomes ; i++) {
    for (j=i+1 ; j<num_genomes ; j++) {
      if (CIRCULAR)
	D[i][j] = (double)invdist_circular(genome_list+i, genome_list+j,
					   num_genes, distmem);
      else 
	D[i][j] = (double)invdist_noncircular(genome_list+i, genome_list+j, 0,
					      num_genes, distmem);
    }
  }
  ...
  if (MYPROC == 0) {
#endif
    fprintf(outfile,"Number of neighbor-joining trees evaluated: %12d\n\n",
	    count);
    fflush(outfile);
#ifdef MPBPA
  }
