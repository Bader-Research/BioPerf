  ...
  for ( k = 1; k <= 3; k++ ) 
    {
      ...
      for ( l = k + 1; l <= 3; l++ )
        rdist[k][l] = rdist[l][k] = 
              invdist_noncircular_nomem(gen[k],gen[l],0,Num_Genes);
    }
}
...
int median_distance(int *med)
{
  ...
  ldist = 0;
  for ( k = 1; k <= 3; k++ )
    ldist += invdist_noncircular_nomem(gen[k],genupd,0,Num_Genes);
 
  return(ldist);
}

int* Find_circular_identity(int* perm) 
{
  int i, j, *id;
  int sign = 1;
  ...
  if ( i == Num_Genes ) 
    {
      fprintf(stderr,"FATAL ERROR: permutation has no element labeled '1'.\n");
      exit(1);
    }
  if (sign == 1) 
    {
...
#define K 1
  ...
  for ( e = 0; e < mc; e++ ) 
    {
      i = i_ind[pmi[e]]; j = j_ind[pmi[e]];
      pme[i] = pme[j] = pmi[e];
      pmmate[i] = j; pmmate[j] = i;
      i = i_ind[pm[k][e]]; j = j_ind[pm[k][e]];
      ke[i] = ke[j] = pm[k][e];
      kmate[i] = j; kmate[j] = i;
    }
...
void msbrbb(int lncyc, int ln, int lub, int lcurr)
{
  ...
  if ( treenodes % MILL == 0 ) 
    {
      millnodes++;
      treenodes = 0;
      if ( tf - ti > TLIM ) termin(tf - ti,FALSE);
      stop_flag = TRUE; 
    }
  ...
  /* recursion exit */
  if ( nunreach == 0 ) return;
  /* upper bound test */
  if ( lub <= best_now ) return;
  ...
  for ( i = 0; i < nunreach; i++ ) 
    {
      ...
      if ( hamate[lcurr] == lnext && nunreach > 1 ) continue;
      ...
      for ( k = 1; k <= 3; k++ ) 
        for ( l = k + 1; l <= 3; l++ ) 
	  {
            if ( permate[k][lcurr] == lnext && permate[l][lcurr] == lnext )
              /* 1 cycle before contraction and 0 after */
              dcycles[k][l] = -1;
            else if(permate[k][lcurr] == lnext || permate[l][lcurr] == lnext) 
      ...
      for ( k = 1; k <= 3; k++ ) 
	{
      ...
      newub = lncyc + newcyc + 
             (int)( totcycles / (double) ( 3 - 1 ) + (double)(3 * mc) / 2.0 );
   ...
   for(i=0; i<4; i++) free(dcycles[i]); free(dcycles); 
  }
}
...
void init_global_variables(int N)
{
  ...
  id = (struct genome_struct**)calloc(MAXQ, sizeof(struct genome_struct*)); 
  for(i=0; i<MAXQ; i++)  
    {
      id[i] = (struct genome_struct*)malloc(sizeof(struct genome_struct));
    } 
...
/*  pibest = (int*)calloc(Num_Genes, sizeof(int));*/
  ...
  gen = (struct genome_struct**)calloc(MAXQ+1, sizeof(struct genome_struct*));
  for(i=0; i<MAXQ+1; i++)
    {
      gen[i] = (struct genome_struct*)malloc(sizeof(struct genome_struct));
...
{
  int i;
  for(i=0; i<MAXQ+1; i++)  free(pi[i]);  free(pi);
  for(i=0; i<MAXQ+1; i++)  free(pm[i]);  free(pm);
  free(i_ind);  free(j_ind); 
  for(i=0; i<MAXNOD; i++)  free(e_ind[i]);  free(e_ind);

  free(unreach);  free(inv_unreach);   
  for(i=0; i<MAXQ+1; i++)
    {  free(ncycles[i]);  free(rdist[i]);  free(permate[i]);  }
  free(ncycles); free(rdist); free(permate);
  free(hamilmate); free(hamate); free(solmate); free(piupd); 

  for(i=1; i<MAXQ; i++) { free(gen[i]->genes); free(gen[i]); }  free(gen);
  free(genupd->genes);  free(genupd);
  free(pme); free(ke); free(pmmate); free(kmate); free(unexp); free(inv_unexp);
  
}

int* albert_inversion_median_circular(struct genome_struct **passgenome, int ngenes, int* genes)
{
  int *med, i;
  ...
  for(i=0; i<MAXQ; i++)  
    {
      id[i]->genes = Find_circular_identity(passgenome[i]->genes);
    } 
...
int* albert_inversion_median_noncircular(struct genome_struct **passgenome, int ngenes, int* genes)
{
  ...
  for(j=1; j<MAXQ+1; j++)
    for ( i = 0; i < Num_Genes; i++ )    pi[j][i] = passgenome[j-1]->genes[i];
  ...
  for ( k = 1; k <= 3; k++ ) 
	...
	{
          if ( h < Num_Genes )   curr = pi[k][h];
          else   curr = Num_Genes + 1;
          if ( curr >= 0 && prev >= 0 ) 
	    {
  ...
  for ( i = 0; i < vc; i++ )   hamilmate[i] = hamate[i];  
  /* solution edges */
  for ( i = 0; i < vc; i++ )   solmate[i] = NONE;
  ...
  UB = (int) ( totcycles / (double) ( 3 - 1 ) + (double) ( 3 * mc ) / 2.0 );
