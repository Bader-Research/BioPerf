...
void calc_invmatrix_BH(struct genome_struct *genomes, int num_genes, int num_genomes,
		       distmem_t *distmem, int CIRCULAR) {
  int i, j;
  int dist;
  ...
  for (i=0 ; i<num_genomes ; i++) {
    for (j=i+1 ; j<num_genomes ; j++) {
      if (CIRCULAR)
	dist = invdist_circular_BH(genomes+i, genomes+j, num_genes, distmem);
      else 
	dist = invdist_noncircular_BH(genomes+i, genomes+j, 0, num_genes, distmem);
    }
  }
...
int num_cycles(int *perm, int size, distmem_t *distmem) {
  int *done;
  int i, ind, j1, j2;
  int next;
  int c;
...
int num_cycles_BH(int *perm, int size, distmem_t *distmem) {
  int *done;
  int i, ind, j1, j2;
  int next;
  int c;
			 ...
			 int *num_components)
{
  ...
  *num_components = 0;
  ...
  for (i=0 ; i<size ; i++) {
    if (greyEdges[i]==-1)  continue;  /*it is self loop,discard it*/
    if (parent[i]==i) {
      stack_ptr++;   
...
void num_hurdles_and_fortress(int *perm, int size,
			      ...
			      distmem_t *distmem)
{
  ...
  */
  ...
  for (i=0 ; i<num_components ; i++) {
    if ((components[i].oriented == FALSE) && (components[i].blocks == 1)) {
#ifdef DEBUG
      fprintf(stdout,"nonoriented component %3d: blocks: %3d\n",
	      i,components[i].blocks);
#endif
  ...
  for (i=0 ; i<num_components ; i++) {
	...
	if ((components[components[i].left].blocks == 2) &&
	    ((components[components[i].left].hurdle & GREATHURDLE) == 0)) {
#ifdef DEBUG
  ...
  if ((*num_hurdles == num_superhurdles) && (num_superhurdles % 2 == 1))
    *num_fortress = 1;

  return;
}

void connected_component_BH(int size,distmem_t *distmem, int *num_components)
{
  UFelem *uf;
  int *stack;
  int stackPtr;
    ...
    if (j != -1) {
      if (i < j) {
        ...
        while ((stackPtr>0) && (stack[stackPtr-1] >= s)) {
          /* C <- Union(Find(pop), C) */
          cIdx = UFunion(uf, UFfind(uf, stack[stackPtr-1]), cIdx);
          stackPtr--;
        }
        /* e <- handle(C) */
        eB = uf[cIdx].handleB;
  ...
  for (i=0 ; i<size ; i++) {
    ...
    if (j != -1 ) {
      if (i < j) {
        cIdx = UFfind(uf, i); 
        fprintf(stdout,"Grey Edge (%3d, %3d): component: %3d handle (%3d,%3d)\n",perm[i], perm[j],cIdx,perm[uf[cIdx].handleB], perm[uf[cIdx].handleE]);
      } 
    }
  }
#endif
...
void num_hurdles_and_fortress_BH(int *perm, int size,
				 int *num_hurdles, int *num_fortress,
				 distmem_t *distmem)
{
  int cIdx;
			...
			int offset, int num_genes, distmem_t *distmem)
{
  ...
  num_hurdles_and_fortress(perm, n, &num_hurdles, &num_fortress, distmem);
...
int invdist_noncircular_BH(struct genome_struct *g1, struct genome_struct *g2,
			   int offset, int num_genes, distmem_t *distmem)
{
  int i, twoi, n;
  int b, c;
  ...
  num_hurdles_and_fortress_BH(perm, n, &num_hurdles, &num_fortress, distmem);
  reversal_dist = b - c + num_hurdles + num_fortress;
